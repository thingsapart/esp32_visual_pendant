...
def add_target_if_missing(name, **kwargs):
  existing = env.get("__PIO_TARGETS", {})
  if name in existing:
    print(f'[analyze_elf] target "{name}" already registered; skipping')
    return
  env.AddTarget(name=name, **kwargs)
...
add_target_if_missing(
  ...
  actions = '$PYTHONEXE $PROJECT_DIR/tools/elf-size-analyze.py $BUILD_DIR/${PROGNAME}${PROGSUFFIX} -t $PROJECT_PACKAGES_DIR\\toolchain-xtensa32\\bin\\xtensa-esp32-elf- -a -H -w 120 -R -m 512',
  title = "Analyze RAM",
...
add_target_if_missing(
  "analyze_flash",
  dependencies = "$BUILD_DIR/${PROGNAME}${PROGSUFFIX}",
  actions = '$PYTHONEXE $PROJECT_DIR/tools/elf-size-analyze.py $BUILD_DIR/${PROGNAME}${PROGSUFFIX} -t $PROJECT_PACKAGES_DIR\\toolchain-xtensa32\\bin\\xtensa-esp32-elf- -a -H -w 120 -F -m 512',
  title = "Analyze Flash",
  description = "Build and analyze",
  group="Analyze"
)
