...
try:
    ...
    # Provide a minimal fallback so the script can be executed standalone
    import os as _os
...
def run():
    ...
    # Locate possible panel library roots under .pio/libdeps/*/ and project lib/
    libdeps_base = os.path.join(proj, ".pio", "libdeps")
    if not os.path.isdir(libdeps_base):
        print(f"[patch] libdeps base not present yet: {libdeps_base}, skipping copy")
        return

    panel_lib_glob = os.path.join(libdeps_base, "*", "ESP32_Display_Panel*")
    targets = glob.glob(panel_lib_glob)
    if not targets:
        # Fallback: also check the project's shared `lib` dir for a vendored copy
        shared_dir = os.path.join(proj, "lib")
        targets = glob.glob(os.path.join(shared_dir, "ESP32_Display_Panel*"))
        if not targets:
            print(f"[patch] no ESP32_Display_Panel library found under {libdeps_base} or {shared_dir}")
            return
