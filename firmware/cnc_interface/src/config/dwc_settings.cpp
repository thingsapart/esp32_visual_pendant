...
bool dwc_settings_load() {
  ...
  preferences.getString(KEY_PASSWORD, g_settings.password,
                        sizeof(g_settings.password));
  preferences.getString(KEY_HOST, g_settings.host, sizeof(g_settings.host));

  preferences.end();
  g_loaded = true;
  LOGI(TAG, "Loaded settings: SSID='%s', Host='%s'", g_settings.ssid,
       g_settings.host);
  return true;
#else
...
bool dwc_settings_save(const dwc_settings_t* settings) {
  ...
  if (success) {
    LOGI(TAG, "Saved settings: SSID='%s', Host='%s'", settings->ssid,
         settings->host);
...
const dwc_settings_t* dwc_settings_get() { return &g_settings; }
