...
static void ui_action_handler(const char* action_name, binding_value_t value, void* user_data) {
    ...
    bool is_homing_dialog = machine->message_box && machine->message_box->seq == 99;
    if (is_homing_dialog) {
        ...
        on_dialogs_change(machine, interface); // Trigger UI update to hide the dialog
    ...
    // --- Active Axis Selection ---
        ...
        if (axis_idx < 0){
            LOGI(TAG, "action '%s' => invalid axis %c (%d)", action_name, axis_char, axis_idx);
        ...
        if (!machine->axes_homed[axis_idx]) {
            ...
            char text_buf[128];
            snprintf(text_buf, sizeof(text_buf), "Axis %c is not homed. Home all axes now?", axis_char);
            machine->message_box->text = strdup(text_buf);
            machine->message_box->mode = MESSAGE_OK_CANCEL; // "OK" maps to Yes, "Cancel" to No
            machine->message_box->seq = 99; // Use a special sequence to identify this dialog
            
            // Manually trigger the dialog update since this is a UI-initiated state change
            ...
            LOGI(TAG, "action '%s' => curr axis %d => next %d", action_name, current_axis, new_axis);
