...
typedef struct tab_probe_t {
  ...
  // Nested structs
  probe_btn_matrix_t
      *btns_2d_in;  // Use pointers, as these will be created later
  probe_btn_matrix_t *btns_2d_out;
  probe_btn_matrix_t
      *btns_3d;  // will implement at the very end, let's get 2d working first.
...
// Function prototypes
tab_probe_t *tab_probe_create(lv_obj_t *tabv, struct interface_t *interface,
                              lv_obj_t *tab);
...
void tab_probe_init_probe_tab_3d(tab_probe_t *tp,
                                 lv_obj_t *tab3d);  // will implement last.
void tab_probe_init_axis_float_btn(tab_probe_t *tp);

probe_btn_matrix_t *probe_btn_matrix_create(
    lv_obj_t *parent, tab_probe_t *tab_probe, const probe_action_t (*actions)[],
    size_t num_rows, size_t num_cols, float rect_width_percent,
    float rect_height_percent, bool quick);
