...
static probe_setting_t probe_settings[] = {
    ...
    {"WCS", 0, 0, 0, 'W', NULL,
     false},  // Special case: WCS is handled differently
    {"Quick Mode", 0, 0, 0, 'Q', NULL,
     false}  // Special case: Quick Mode (0 or 1)
};
static const size_t num_probe_settings =
    sizeof(probe_settings) / sizeof(probe_settings[0]);
...
#define img_arr_s_o_data "S:/img/img_arr_s_o.png"
#define img_arr_e_o_data "S:/img/img_arr_e_o.png"
#define img_arr_w_o_data "S:/img/img_arr_w_o.png"
#define img_arr_n_o_data "S:/img/img_arr_n_o.png"
#define img_arr_se_o_data "S:/img/img_arr_se_o.png"  // Back-left
#define img_arr_sw_o_data "S:/img/img_arr_sw_o.png"  // Back-right
#define img_arr_ne_o_data "S:/img/img_arr_ne_o.png"  // Front-Left
#define img_arr_nw_o_data "S:/img/img_arr_nw_o.png"  // Front-right
...
LV_IMG_DECLARE(img_arr_s_o)
LV_IMG_DECLARE(img_arr_e_o)
LV_IMG_DECLARE(img_arr_w_o)
LV_IMG_DECLARE(img_arr_n_o)
// LV_IMG_DECLARE(img_ref_sfc)
LV_IMG_DECLARE(img_arr_se_o)  // Back-left
LV_IMG_DECLARE(img_arr_sw_o)  // Back-right
LV_IMG_DECLARE(img_arr_ne_o)  // Front-Left
LV_IMG_DECLARE(img_arr_nw_o)  // Front-right
LV_IMG_DECLARE(img_pkt_in_o)  // Outside Rect
...
#define img_arr_s_o_data &img_arr_s_o
#define img_arr_e_o_data &img_arr_e_o
#define img_arr_w_o_data &img_arr_w_o
#define img_arr_n_o_data &img_arr_n_o
#define img_arr_se_o_data &img_arr_se_o  // Back-left
#define img_arr_sw_o_data &img_arr_sw_o  // Back-right
#define img_arr_ne_o_data &img_arr_ne_o  // Front-Left
#define img_arr_nw_o_data &img_arr_nw_o  // Front-right
#endif

// 3-Axis probing operations.
static const char *probe_modes_3d_bl[] = {
    "Q", "W", "P:$Z", "N:3", "J:" PRB_J, "K:" PRB_K, "L:" PRB_L,
    "H", "I", "T",    "C",   "O",        NULL};
static const char *probe_modes_3d_br[] = {
    "Q", "W", "P:$Z", "N:2", "J:" PRB_J, "K:" PRB_K, "L:" PRB_L,
    "H", "I", "T",    "C",   "O",        NULL};
static const char *probe_modes_3d_fl[] = {
    "Q", "W", "P:$Z", "N:0", "J:" PRB_J, "K:" PRB_K, "L:" PRB_L,
    "H", "I", "T",    "C",   "O",        NULL};
static const char *probe_modes_3d_fr[] = {
    "Q", "W", "P:$Z", "N:1", "J:" PRB_J, "K:" PRB_K, "L:" PRB_L,
    "H", "I", "T",    "C",   "O",        NULL};

// 2-axis Probing Operations: inside-towards-outside.
static const char *probe_modes_2d_in_params_bl[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:3", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_in_params_br[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:2", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_in_params_fl[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:0", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_in_params_fr[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:1", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_in_params_pkt[] = {
    "Q", "W", "N", "O", "J", "K", "L", "H", "I", "T", NULL};
static const char *probe_modes_2d_in_params_bore[] = {"Q", "W", "N", "O", "J",
                                                      "K", "L", "H", NULL};

// 2-axis Probing Operations: outside-towards-inside.
static const char *probe_modes_2d_out_params_bl[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:3", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_out_params_br[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:2", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_out_params_fl[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:0", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_out_params_fr[] = {
    "Q",        "W",        "Z:" PRB_Z, "N:1", "O", "J:" PRB_J,
    "K:" PRB_K, "L:" PRB_L, "H",        "I",   "T", NULL};
static const char *probe_modes_2d_out_params_rect[] = {
    "Q", "W", "N", "O", "J", "K", "L", "H", "I", "T", NULL};
static const char *probe_modes_2d_out_params_boss[] = {"Q", "W", "N", "O", "J",
                                                       "K", "L", "H", NULL};

// Single-axis, single-surface Probing.
static const char *surface_probe_back_params[] = {"W", "H", "I", "J",
                                                  "K", "L", NULL};
static const char *surface_probe_left_params[] = {"W", "H", "I", "J",
                                                  "K", "L", NULL};
static const char *surface_probe_top_params[] = {"W", "H", "I", "J",
                                                 "K", "L", NULL};
static const char *surface_probe_right_params[] = {"W", "H", "I", "J",
                                                   "K", "L", NULL};
static const char *surface_probe_front_params[] = {"W", "H", "I", "J",
                                                   "K", "L", NULL};
static const char *surface_probe_ref_params[] = {
    NULL};  // Empty - no parameters

static const probe_action_t probe_modes_3d[][2] = {
    {
        {"G6520.1", probe_modes_3d_bl, img_arr_nw_o_data,
         "back-left vise corner"},  // Back-Left
        {"G6520.1", probe_modes_3d_br, img_arr_ne_o_data,
         "back-right vise corner"},  // Back-Right
    },
    {
        {"G6520.1", probe_modes_3d_fl, img_arr_sw_o_data,
         "front-left vise corner"},  // Front-Left
        {"G6520.1", probe_modes_3d_fr, img_arr_se_o_data,
         "front-right vise corner"},  // Front-Right
    },
};
static const size_t probe_modes_3d_rows =
    sizeof(probe_modes_3d) / sizeof(probe_modes_3d[0]);
static const size_t probe_modes_3d_cols =
    sizeof(probe_modes_3d[0]) / sizeof(probe_action_t);

static const probe_action_t probe_modes_2d_out[][3] = {
    {
        {"G6509.1", probe_modes_2d_out_params_bl, img_arr_nw_o_data,
         "outside back-left corner"},  // Back-Left
        {"G6520.1", surface_probe_front_params, img_arr_n_o_data,
         "outside back face"},  // Outside back = inside front
        {"G6509.1", probe_modes_2d_out_params_br, img_arr_ne_o_data,
         "outside back-right corner"},  // Back-Right
    },
    {
        {"G6520.1", surface_probe_right_params, img_arr_w_o_data,
         "outside left face"},  // Outside left = Inside right
        {"G6501.1", probe_modes_2d_out_params_boss, img_ctr1_boss_data,
         "center boss (outside)"},  // Outside Boss
        {"G6520.1", surface_probe_left_params, img_arr_e_o_data,
         "outside right face"},  // Outside right = inside left
    },
    {
        {"G6509.1", probe_modes_2d_out_params_fl, img_arr_sw_o_data,
         "outside front-left corner"},  // Front-Left
        {"G6520.1", surface_probe_back_params, img_arr_s_o_data,
         "outside front face"},  // Outside front = inside back
        {"G6509.1", probe_modes_2d_out_params_fr, img_arr_se_o_data,
         "front-right corner"},  // Front-Right
    },
    /* {
        {"G6503.1", probe_modes_2d_out_params_rect, img_pkt_in_data, "outside
    rectangle"}, // Outside Rectangle
    }*/
};
static const size_t probe_modes_2d_out_rows =
    sizeof(probe_modes_2d_out) / sizeof(probe_modes_2d_out[0]);
static const size_t probe_modes_2d_out_cols =
    sizeof(probe_modes_2d_out[0]) / sizeof(probe_action_t);

// Create the 2D array (an array of row pointers)
static const probe_action_t probe_modes_2d_in[][3] = {
    {
        {"G6508.1", probe_modes_2d_in_params_bl, img_arr_nw_data,
         "back-left inside corner"},  // Back-Left
        {"G6520.1", surface_probe_back_params, img_arr_n_data,
         "inside back face"},  // Outside front = inside back
        {"G6508.1", probe_modes_2d_in_params_br, img_arr_ne_data,
         "back-right inside corner"},  // Back-Right
    },
    {
        {"G6520.1", surface_probe_left_params, img_arr_w_data,
         "inside left face"},  // Outside right = inside left
        {"G6500.1", probe_modes_2d_in_params_bore, img_ctr1_bore_data,
         "center bore (inside)"},  // Inside Bore
        {"G6520.1", surface_probe_right_params, img_arr_e_data,
         "inside right face"}  // Outside left = Inside right
    },
    {
        {"G6508.1", probe_modes_2d_in_params_fl, img_arr_sw_data,
         "front-left inside corner"},  // Front-Left
        {"G6520.1", surface_probe_front_params, img_arr_s_data,
         "insider front face"},  // Outside back = inside front
        {"G6508.1", probe_modes_2d_in_params_fr, img_arr_se_data,
         "front-right inside corner"},  // Front-Right
    },
    /* {
        {"G6502.1", probe_modes_2d_in_params_pkt, img_pkt_out_data, "inside
    pocket"},          // Inside Pocket
    }*/
};

static const size_t probe_modes_2d_in_rows =
    sizeof(probe_modes_2d_in) / sizeof(probe_modes_2d_in[0]);
static const size_t probe_modes_2d_in_cols =
    sizeof(probe_modes_2d_in[0]) / sizeof(probe_action_t);

static const probe_action_t surface_probe_modes[][2] = {
    {
        {"G6520.1", surface_probe_top_params, img_ctr1_boss_data,
         "top surface"},  // Top surface
        {"G6520.1", surface_probe_ref_params, img_ctr1_bore_data,
         "reference surface"}  // ref surface
    },
};

static const size_t surface_probe_modes_rows =
    sizeof(surface_probe_modes) / sizeof(surface_probe_modes[0]);
static const size_t surface_probe_modes_cols =
    sizeof(surface_probe_modes[0]) / sizeof(probe_action_t);

// --- Helper Functions ---

// Create a string representation of a float, handling NaN
static void float_to_string(float value, char *buffer, size_t buffer_size,
                            int precision) {
  if (isnan(value)) {
...
float setting_from_param(const char param_name, tab_probe_t *tp) {
  if (param_name == 'H')
    return (tp->settings.width_dia);
  else if (param_name == 'I')
    return (tp->settings.length);
  else if (param_name == 'Z')
    return (tp->settings.depth_dist);
  else if (param_name == 'T')
    return (tp->settings.surf_clear);
  else if (param_name == 'C')
    return (tp->settings.corner_clear);
  else if (param_name == 'O')
    return (tp->settings.overtravel);
  else if (param_name == 'W')
    return (tp->settings.wcs);
  else if (param_name == 'Q')
    return (tp->settings.quick_mode);
  else
    return NAN;
}
...
char *make_param_value(const char *param_desc, tab_probe_t *tp) {
  ...
  printf("'%s' :: len > 2? => %d, %c (desc[1] == ':'? => %d\n", param_desc,
         strlen(param_desc) > 2, param_desc[1], param_desc[1] == ':');
  ...
  for (size_t j = 0; j < num_probe_settings; j++) {
    if (strlen(param_desc) == 1 &&
        param_desc[0] == probe_settings[j].param_name) {
      printf("VAR: ");
...
char *replace_vars(const char *param_desc, tab_probe_t *tp) {
  ...
  assert((strlen(param_desc) < MAX_PARAM_DESC_LEN) &&
         "Expected param desc < MAX_PARAM_DESC_LEN");
  ...
  while (*param_desc != '\0' && (cnext = strchr(param_desc, '$')) != NULL) {
    ...
    assert((len > next + 1) &&
           "Expected replacement expr $ to be followed by setting/param.");
    strncpy(buf + offs, param_desc, next);
    offs += next - offs;
    char pdesc[2];
    pdesc[0] = param_desc[next + 1];
    pdesc[1] = '\0';
...
static void make_gcode(char *buffer, size_t buffer_size, const char *gcode,
  ...
  for (int i = 0; params[i] != NULL; i++) {
    ...
    if (value) {
      offset += snprintf(buffer + offset, buffer_size - offset, "%c%s ",
                         param_desc[0], value);
      free(value);
    } else {
      ...
      offset +=
          snprintf(buffer + offset, buffer_size - offset, "%s? ", param_desc);
    }
  }
}
...
static void probe_btn_matrix_click_handler(lv_event_t *e) {
  ...
  if (!pbm->tab_probe->interface->machine->is_connected(
          pbm->tab_probe->interface->machine)) {
    message_modal("Machine not connedted",
                  "Could not establish connection to machine.\n\nCheck wiring "
                  "and make sure machine is in valid state.");
    return;
  }

  if (!machine_interface_is_homed(pbm->tab_probe->interface->machine, NULL)) {
    // ui.modals.home_modal(pbm->tab_probe->interface); // You'll need a C
    // version of this
    LV_LOG_WARN(
        "Machine is not homed or not connected.");  // Placeholder: Show a modal
  ...
  make_gcode(gcode_buffer, sizeof(gcode_buffer), action->gcode, action->params,
             pbm->tab_probe);
    ...
    if (strlen(p) == 1) {
      for (int j = 0; j < num_probe_settings; j++) {
        if (p[0] == probe_settings[j].param_name) {
          ...
          buf += snprintf(buf, sizeof(modal_text) - (buf - modal_text) - 1,
                          "%s: %s\n", probe_settings[j].label, v);
          free(v);
          break;
        }
      }
  ...
  *(buf + 1) = '\0';
  ...
  void (*btn_cbs[])(lv_event_t * e) = {probe_msg_box_probe, modal_close_handler,
                                       NULL};
...
static void quick_mode_cb(lv_event_t *e) {
  ...
  if (pbm && pbm->tab_probe) {
    pbm->tab_probe->settings.quick_mode =
        lv_obj_has_state(sw, LV_STATE_CHECKED) ? 1 : 0;
  }
}
...
void style_probe_obj(lv_obj_t *obj, float w_percent, float h_percent) {
  _style_gradient(
      obj, lv_color_hex(0xCCCCCC55), lv_color_hex(0x77777755), LV_GRAD_DIR_VER,
      200, 2, lv_palette_lighten(LV_PALETTE_AMBER, 3), 5,
...
probe_btn_matrix_t *probe_btn_matrix_create(
    lv_obj_t *parent, tab_probe_t *tab_probe,
    const probe_action_t (*actions_)[], size_t num_rows, size_t num_cols,
    float rect_width_percent, float rect_height_percent, bool quick) {
  probe_btn_matrix_t *pbm =
      (probe_btn_matrix_t *)malloc(sizeof(probe_btn_matrix_t));
  if (!pbm) {
  ...
  _margins(pbm->container, 0, 10, 0, 10);
  ...
  for (size_t j = 0; j < num_rows; j++) {
    ...
    _maximize_client_area(row_container);
    _bg_opa(row_container, LV_OPA_0, _M);
    _pad_column(row_container, 1);
    _pad_row(row_container, 1);
    ...
    for (size_t i = 0; i < num_cols; i++) {
      ...
      if (action->gcode == NULL) {
        ...
        _size(btn, PROBE_BTN_WIDTH, PROBE_BTN_HEIGHT);
        _margin(btn, 0);
        _pad(btn, 0);

        lv_obj_add_event_cb(btn, probe_btn_matrix_click_handler,
        ...
        _style_local(btn, border_color, LV_PART_MAIN,
                     lv_color_hex(0x008080));  // Teal
  ...
  if (quick) {
    ...
    lv_checkbox_set_text(pbm->quick_mode_chk, "Skew calibration");
    lv_obj_center(pbm->quick_mode_chk);
    lv_obj_add_event_cb(pbm->quick_mode_chk, quick_mode_cb,
                        LV_EVENT_VALUE_CHANGED, pbm);  // Pass pbm as user_data
  ...
  if (pbm->quick_mode_chk) {
    lv_obj_align_to(pbm->quick_mode_chk, pbm->container, LV_ALIGN_BOTTOM_LEFT,
                    0, 50);
    _pad_bottom(pbm->container, 15, _M);
  }

// Draw a background representing the probed box' borders?
#ifdef PROBE_BOX_BG
  mk_container(NULL, pbm->container, lv_coord_t w = width_(pbm->container),
               h = height_(pbm->container);

               _size(obj, w, h); _maximize_client_area(obj);
               _use_layout(obj, false); _update_layout(obj); _center(obj);
...
static void slider_event_cb(lv_event_t *e) {
  ...
  for (i = 0; i < num_probe_settings; i++) {
    if (slider == probe_settings[i].slider) {
      break;
    }
  }
  if (i >= num_probe_settings || !slider) {
    return;
  }
  ...
  if (textbox && tp) {
    ...
    if (probe_settings[i].param_name == 'H')
      tp->settings.width_dia = value;
    else if (probe_settings[i].param_name == 'I')
      tp->settings.length = value;
    else if (probe_settings[i].param_name == 'Z')
      tp->settings.depth_dist = value;
    else if (probe_settings[i].param_name == 'T')
      tp->settings.surf_clear = value;
    else if (probe_settings[i].param_name == 'C')
      tp->settings.corner_clear = value;
    else if (probe_settings[i].param_name == 'O')
      tp->settings.overtravel = value;
  }
}
    ...
    if (strcmp(lv_label_get_text(label), probe_settings[i].label) == 0) {
      ...
      if (slider) {
        lv_slider_set_value(slider,
                            (int)(probe_settings[i].default_value * 10.0f),
                            LV_ANIM_OFF);
...
tab_probe_t *tab_probe_create(lv_obj_t *tabv, interface_t *interface,
                              lv_obj_t *tab) {
  ...
  tp->settings.wcs =
      (int)probe_settings[6].default_value;  // Cast to int.  This is an INDEX.
  tp->settings.quick_mode = (int)probe_settings[7].default_value;  // cast to
                                                                   // int
...
static void tab_probe_update_quick(lv_event_t *e) {
  ...
  if (tp->settings.quick_mode == 1) {
    if (tp->btns_2d_in)
      lv_obj_add_state(tp->btns_2d_in->quick_mode_chk, LV_STATE_CHECKED);
    if (tp->btns_2d_out)
      lv_obj_add_state(tp->btns_2d_out->quick_mode_chk, LV_STATE_CHECKED);
    if (tp->btns_3d)
      lv_obj_add_state(tp->btns_3d->quick_mode_chk,
                       LV_STATE_CHECKED);  // will add.
  } else {
    if (tp->btns_2d_in)
      lv_obj_clear_state(tp->btns_2d_in->quick_mode_chk, LV_STATE_CHECKED);
    if (tp->btns_2d_out)
      lv_obj_clear_state(tp->btns_2d_out->quick_mode_chk, LV_STATE_CHECKED);
    if (tp->btns_3d)
      lv_obj_clear_state(tp->btns_3d->quick_mode_chk,
                         LV_STATE_CHECKED);  // will add.
  }
}
...
void tab_probe_init_probe_tabv(tab_probe_t *tp, lv_obj_t *parent) {
  ...
  tp->tab_probe_2d_in = lv_tabview_add_tab(tp->main_tabs, "Inside");
  tp->tab_probe_2d_out = lv_tabview_add_tab(tp->main_tabs, "Outside");
  tp->tab_surf = lv_tabview_add_tab(tp->main_tabs, "Extra");

  lv_obj_add_event_cb(tp->main_tabs, tab_probe_update_quick,
                      LV_EVENT_VALUE_CHANGED, tp);  // tp as user data

  tab_probe_init_sets_tab(tp, tp->tab_settings);
  tab_probe_init_probe_tab_2d_in(tp, tp->tab_probe_2d_in);
  tab_probe_init_probe_tab_2d_out(tp, tp->tab_probe_2d_out);
...
void tab_probe_init_sets_tab(tab_probe_t *tp, lv_obj_t *tab) {
  ...
  // Define columns: [label_width, textbox_width, slider_width,
  // LV_GRID_TEMPLATE_LAST]
  lv_coord_t _cols[] = {100, 60, lv_grid_fr(1), LV_GRID_TEMPLATE_LAST};
  // Define rows based on the number of settings
  lv_coord_t *rows =
      (lv_coord_t *)malloc((num_probe_settings + 1) * sizeof(lv_coord_t));
  if (!rows) {
  ...
  if (!cols) {
    LV_LOG_ERROR("failed to alloc cols array");
    return;
  }
  ...
  for (size_t i = 0; i < num_probe_settings; i++) {
    if (probe_settings[i].default_value != NAN)  // Only for numerical settings
    {
      ...
      lv_textarea_set_text(textbox, buf);
      lv_obj_set_grid_cell(textbox, LV_GRID_ALIGN_STRETCH, 1, 1,
                           LV_GRID_ALIGN_CENTER, i, 1);
      ...
      lv_slider_set_range(slider, (int)(probe_settings[i].min_value * 10.0f),
                          (int)(probe_settings[i].max_value * 10.0f));
      lv_slider_set_value(
          slider, (int)(probe_settings[i].default_value * 10.0f), LV_ANIM_OFF);

      lv_obj_set_grid_cell(slider, LV_GRID_ALIGN_STRETCH, 2, 1,
                           LV_GRID_ALIGN_CENTER, i, 1);
      ...
      // store tab_probe in slider and textbox so we can access the settings and
      // update them.
...
static void cb_set_wcs(lv_event_t *e) {
  ...
  if (obj && tp) {
    tp->settings.wcs =
        lv_btnmatrix_get_selected_btn(obj);  // Get selected *index*
    printf("WCS: %d\n\n", tp->settings.wcs);
  }
}

static const char *wcs_map[] = {"G54", "G55", "G56",   "\n",    "G57",   "G58",
                                "G59", "\n",  "G59.1", "G59.2", "G59.3", NULL};
...
void tab_probe_init_wcs_tab(tab_probe_t *tp, lv_obj_t *tab) {
  ...
  lv_btnmatrix_set_btn_ctrl(tp->wcs_buttons, 0,
                            LV_BTNMATRIX_CTRL_CHECKED);  // G54 checked
  lv_obj_add_event_cb(tp->wcs_buttons, cb_set_wcs, LV_EVENT_CLICKED,
                      tp);  // Pass tp as user_data
...
void tab_probe_init_surface_tab(tab_probe_t *tp, lv_obj_t *tab) {
  ...
  tp->btns_surf = probe_btn_matrix_create(
      container, tp, surface_probe_modes, surface_probe_modes_rows,
      surface_probe_modes_cols, 0, 0, false);
  if (!tp->btns_surf) {
    LV_LOG_ERROR("Failed to create surface probe matrix");
  }
}

void tab_probe_init_probe_tab_2d_out(tab_probe_t *tp, lv_obj_t *tab) {
  _style_local(tab, pad_all, LV_PART_MAIN, 5);
  _style_local(tab, margin_all, LV_PART_MAIN, 0);
  _scrollable(tab, false);
  ...
  tp->btns_2d_out = probe_btn_matrix_create(
      container, tp, probe_modes_2d_out, probe_modes_2d_out_rows,
      probe_modes_2d_out_cols, 0, 0, true);
  if (!tp->btns_2d_out) {
    LV_LOG_ERROR("2d in probe matrix alloc failed");
  }
}

void tab_probe_init_probe_tab_2d_in(tab_probe_t *tp, lv_obj_t *tab) {
  _style_local(tab, pad_all, LV_PART_MAIN, 5);
  _style_local(tab, margin_all, LV_PART_MAIN, 0);
  _scrollable(tab, false);
  ...
  tp->btns_2d_in = probe_btn_matrix_create(container, tp, probe_modes_2d_in,
                                           probe_modes_2d_in_rows,
                                           probe_modes_2d_in_cols, 0, 0, true);
  if (!tp->btns_2d_in) {
    LV_LOG_ERROR("2d in probe matrix alloc failed");
  }

  /* Sub-tabview for in/out - no longer enough space with larger icons => move
 left lv_obj_t *tabv = lv_tabview_create(tab2d); _tv_bar_pos(tabv, LV_DIR_TOP);
  ...
  tp->btns_2d_out = probe_btn_matrix_create(tab_out, tp, probe_modes_2d_out,
 probe_modes_2d_out_rows, probe_modes_2d_out_cols, 0.9, 0.75, true);
  if(!tp->btns_2d_out) {
 ...
 _flex_flow(tab_in, LV_FLEX_FLOW_COLUMN);
  tp->btns_2d_in = probe_btn_matrix_create(tab_in, tp, probe_modes_2d_in,
 probe_modes_2d_in_rows, probe_modes_2d_in_cols, 0.5, 0.5, true);
  if(!tp->btns_2d_in) {
      LV_LOG_ERROR("2d in probe matrix alloc failed");
  }

  lv_obj_add_event_cb(tabv, tab_probe_update_quick, LV_EVENT_VALUE_CHANGED, tp);
 // tp as user data
  */
}
...
void tab_probe_init_probe_tab_3d(tab_probe_t *tp, lv_obj_t *tab3d) {
  ...
  tp->btns_3d = probe_btn_matrix_create(container, tp, probe_modes_3d,
                                        probe_modes_3d_rows,
                                        probe_modes_3d_cols, 0.5, 0.5, true);
}
...
static void axis_change_cb(machine_interface_t *mach, void *user_data) {
  ...
  if (float_btn_label) {
    lv_label_set_text(
        float_btn_label,
        axes_options[mach->current_move_axis]);  // get current axis.
  }
}
...
void tab_probe_init_axis_float_btn(tab_probe_t *tp) {
  ...
  lv_label_set_text(
      label,
      axes_options[tp->interface->machine->current_move_axis]);  // set initial
                                                                 // axis.
  lv_obj_center(label);

  lv_obj_add_event_cb(tp->float_btn, axis_float_btn_cb, LV_EVENT_CLICKED,
                      tp);  // tp as user data
  machine_interface_add_current_move_axis_changed_cb(tp->interface->machine,
                                                     label, axis_change_cb);
