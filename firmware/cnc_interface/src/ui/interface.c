...
void _mach_current_move_axis_changed(machine_interface_t *mach,
                                     void *user_data);
...
#define LV_FONT_ASSIGN(dest, font)                  \
  do {                                              \
    const lv_font_t *font_addr = &font;             \
    memcpy(&dest, &font_addr, sizeof(lv_font_t *)); \
  } while (0);
#endif
...
void interface_init_fonts(interface_t *interface) {
  ...
  // const char *base_path = "/path/to/your/assets/font/";  // Replace with your
  // actual path.
...
void interface_init_main_tabs(interface_t *interface) {
  // interface->main_tabs = lv_tabview_create(interface->scr, LV_DIR_TOP,
  // TAB_HEIGHT);
  lv_obj_t *tabv = interface->main_tabs =
      TABV("itf:main_tabv", interface->scr, _tv_bar_pos(obj, LV_DIR_TOP);
...
#ifdef POSIX
  mk_container(
      NULL, interface->scr, _use_layout(obj, false); _size(obj, 100, 30);
      _maximize_client_area(obj); _pos(obj, 0, 0); _flex_row(obj);

      mk_btn(
          NULL, outer_obj, _maximize_client_area(obj);
          _size(obj, lv_pct(40), lv_pct(100));
          mk_label(NULL, outer_obj, _label_text(obj, "+"));
          lv_obj_add_event_cb(obj, jog_plus_cb, LV_EVENT_CLICKED, interface););

      mk_btn(NULL, outer_obj, _maximize_client_area(obj);
             _size(obj, lv_pct(40), lv_pct(100));
             mk_label(NULL, outer_obj, _label_text(obj, "-"));
             lv_obj_add_event_cb(obj, jog_minus_cb, LV_EVENT_CLICKED,
                                 interface);););
#endif
}
...
void interface_register_state_change_cb(interface_t *interface,
  ...
  for (size_t i = 0; i < sizeof(interface->machine_change_cbs) /
                             sizeof(interface->machine_change_cbs[0]);
       i++) {
  ...
  assert(
      false &&
      "Too many machine state update callbacks - update MAX_MACHINE_STATE_CBS");
}
...
void interface_update_machine_state(interface_t *interface,
  ...
  for (size_t i = 0; i < sizeof(interface->machine_change_cbs) /
       ...
       i++) {
    if (interface->machine_change_cbs[i].cb) {
      interface->machine_change_cbs[i].cb(
          interface->machine, interface->machine_change_cbs[i].user_data);
      return;
    }
  }
}
...
void _mach_files_changed(machine_interface_t *mach, void *user_data,
...
#define CHECK_AND_CALL_CBS(cb_name)                                          \
  do {                                                                       \
    ...
    if (interface->machine_state_updated.cb_name##_changed) {                \
      for (size_t i = 0; i < sizeof(interface->cb_name##_changed_cb) /       \
                                 sizeof(interface->cb_name##_changed_cb[0]); \
           ++i) {                                                            \
        _df(-1, #cb_name " CHECKING %d (%p)", i,                             \
            interface->cb_name##_changed_cb[i].cb_fn);                       \
        if (interface->cb_name##_changed_cb[i].cb_fn) {                      \
          _df(-1, #cb_name " CALLING %d (%p)", i,                            \
              interface->cb_name##_changed_cb[i].cb_fn);                     \
          interface->cb_name##_changed_cb[i].cb_fn(                          \
              interface->machine,                                            \
              interface->cb_name##_changed_cb[i].user_data);                 \
        }                                                                    \
      }                                                                      \
...
void interface_tick(interface_t *interface) {
  ...
  for (size_t i = 0; i < MAX_FILE_LISTS; ++i) {
    if (interface->files_changed[i]) {
      ...
      for (size_t j = 0; j < sizeof(interface->files_changed_cb) /
                                 sizeof(interface->files_changed_cb[0]);
           ++j) {
        if (interface->files_changed_cb[j].cb_fn &&
            strcmp(fdir, interface->files_changed_cb[j].path) == 0) {
          interface->files_changed_cb[j].cb_fn(
              interface->machine, interface->files_changed_cb[j].user_data,
              mach->filelists[i].fdir, mach->filelists[i].files);
        }
      }
  ...
  for (size_t i = 0; i < len; ++i) {
    if (s[i] == '<') {
      ...
      if (s[i] == '/') {
        ...
        offs += oi - i + inc;  // dec by length of closing tag plus closing of
                               // formatting ("# ").
      } else {
        const char *sp = &s[i];
...
#define INSERT(str)                      \
  do {                                   \
    const char *colors = str;            \
    const size_t ll = strlen(colors);    \
    for (size_t jj = 0; jj < ll; ++jj) { \
      res[o++] = colors[jj];             \
    }                                    \
  } while (0)
  ...
  switch (m->mode) {
    ...
    case MESSAGE_OK_CANCEL:
      do {
        ...
        const modal_button_cb_t cbs[] = {machine_ok_cb, machine_cancel_cb,
                                         NULL};
        lv_obj_t *res = button_modal(title, ftext, buttons, cbs, m);
        m->user_data = res;
        // lv_obj_add_event_cb(res, modal_delete_cb, LV_EVENT_DELETE, m);
    ...
    default:
      LOGW(TAG,
           "Unsupported message box type: S%d - implementation TODO [M_INFO = "
           "%d, M_OK = %d, M_OK_CANCEL = %d, M_CHOICE = %d].",
           m->mode, MESSAGE_INFO, MESSAGE_OK, MESSAGE_OK_CANCEL,
           MESSAGE_CHOICE);
      break;
  }
}
...
add_callback_fn(interface, state_changed)
  ...
  for (int i = 0; i < MAX_CALLBACKS; i++) {
    if (!self->files_changed_cb[i].cb_fn) {
      ...
      machine_interface_add_files_changed_cb(self->machine, path, self,
                                             _mach_files_changed);
