...
static void label_home_clicked_event_handler(lv_event_t *e) {
  ...
  machine_position_wcs_t *mp =
      (machine_position_wcs_t *)lv_event_get_user_data(e);
  ...
  if (mp) {
    ...
    for (size_t i = 0; i < mp->num_coords; i++) {
      if (label == mp->axis_labels[i]) {
        ...
        const char axis_char[2] = {
            mp->coords[i][0], '\0'};  // Get the first character (e.g., 'X')
        mp->interface->machine->home(
            mp->interface->machine,
...
static void wcs_label_clicked_event_handler(lv_event_t *e) {
  machine_interface_t *iface = (machine_interface_t *)lv_event_get_user_data(
      e);  // get interface from the event.
  _df(-1, "NEXT WCS: %p\n", iface);
...
static void interface_machine_state_updated_cb(machine_interface_t *mach,
                                               void *ud) {
  machine_position_wcs_t *mp = (machine_position_wcs_t *)ud;

  if (!mp || !mp->display_needs_update) {
    return;
  }
  ...
  for (size_t i = 0; i < mp->num_coords; i++) {
    machine_position_wcs_set_coord(mp, i, mach->position[i],
                                   mp->coord_systems[0]);  // "Mach"
  }

  if (mp->num_coord_systems > 1) {
    for (size_t i = 0; i < mp->num_coords; i++) {
      machine_position_wcs_set_coord(mp, i, mach->wcs_position[i],
                                     mp->coord_systems[1]);  // "WCS"
    }
    if (mp->num_coord_systems > 2) {
      for (size_t i = 0; i < mp->num_coords; i++) {
        ...
        {
          machine_position_wcs_set_coord(mp, i, wcs_pos - pos,
                                         mp->coord_systems[2]);  // "Move"
        }
      }
    }
  }
...
static void home_updated_callback(machine_interface_t *mach, void *user_data) {
  ...
  for (size_t i = 0; i < mp->num_coords; i++) {
    lv_color_t colr = mach->axes_homed[i]
                          ? lv_color_hex(0x0000FF)
                          : lv_color_hex(0xFF0000);  // Blue : Red
...
static void wcs_updated_callback(machine_interface_t *mach, void *user_data) {
  ...
  if (mp->num_coord_systems > 1) {
    _label_text(mp->coord_sys_labels[1],
                machine_interface_get_wcs_str(mach, mach->wcs));
  }
}
...
machine_position_wcs_t *machine_position_wcs_create(
    lv_obj_t *parent, const char **coords, size_t num_coords,
    interface_t *interface, int digits, const char **coord_systems,
    size_t num_coord_systems, int coord_sys_width) {
  // --- Argument Validation ---
  ...
  // --- Memory Allocation ---
  machine_position_wcs_t *mp =
      (machine_position_wcs_t *)malloc(sizeof(machine_position_wcs_t));
  if (!mp) {
  ...
  mp->digits = digits;
  snprintf(mp->fmt_str, sizeof(mp->fmt_str), "%%.%df",
           (digits - 2 > 0) ? 2 : 0);
  ...
  mp->coord_sys_labels =
      (lv_obj_t **)malloc(num_coord_systems * sizeof(lv_obj_t *));
  ...
  for (size_t i = 0; i < num_coord_systems; i++) {
    mp->coord_val_labels[i] =
        (lv_obj_t **)malloc(num_coords * sizeof(lv_obj_t *));
  ...
  _style_local(mp->container, bg_opa, LV_PART_MAIN,
               LV_OPA_TRANSP);  // No background.
  ...
  const lv_font_t *font = interface->font_lcd_24
                              ? interface->font_lcd_24
                              : LV_FONT_DEFAULT;  // Assuming you have this font

  int lblc_width = (digits + 1) * FONT_WIDTH;

  // Define grid columns:  [coord_sys_width] [lblc_width] [lblc_width] ...
  // [LV_GRID_TEMPLATE_LAST]
  mp->col_dsc =
      (lv_coord_t *)malloc((1 + num_coord_systems + 1) * sizeof(lv_coord_t));
  if (!mp->col_dsc) {
  ...
  // Define grid rows: [FONT_HEIGHT + 2] [FONT_HEIGHT + 2] ...
  // [LV_GRID_TEMPLATE_LAST]
  ...
  _width(mp->container, coord_sys_width + num_coord_systems * 2 +
                            lblc_width * num_coord_systems + 2);
  ...
  for (size_t i = 0; i < num_coords; i++) {
    ...
    snprintf(label_text, sizeof(label_text), "%s %s", LV_SYMBOL_HOME,
             coords[i]);  // Prepend symbol
    ...
    _style_local(mp->axis_labels[i], text_align, LV_PART_MAIN,
                 LV_TEXT_ALIGN_CENTER);
    lv_obj_set_grid_cell(mp->axis_labels[i], LV_GRID_ALIGN_STRETCH, 0, 1,
                         LV_GRID_ALIGN_STRETCH, i + 1, 1);
    lv_obj_add_event_cb(mp->axis_labels[i], label_home_clicked_event_handler,
                        LV_EVENT_CLICKED, mp);
    ...
    lv_color_t colr = interface->machine->axes_homed[i]
                          ? lv_color_hex(0x0000FF)
  ...
  for (size_t i = 0; i < num_coord_systems; i++) {
    ...
    if (i == 1) {
      lv_obj_add_event_cb(label, wcs_label_clicked_event_handler,
                          LV_EVENT_CLICKED, interface->machine);
  ...
  for (size_t i = 0; i < num_coord_systems; i++) {
    for (size_t j = 0; j < num_coords; j++) {
      ...
      _style_local(mp->coord_val_labels[i][j], bg_color, LV_PART_MAIN,
      ...
      _style_local(mp->coord_val_labels[i][j], text_align, LV_PART_MAIN,
                   LV_TEXT_ALIGN_RIGHT);

      if (font) {
        lv_obj_set_style_text_font(mp->coord_val_labels[i][j], font,
                                   LV_PART_MAIN);
      }
      _width(mp->coord_val_labels[i][j], lblc_width);

      lv_obj_set_grid_cell(mp->coord_val_labels[i][j], LV_GRID_ALIGN_STRETCH,
                           i + 1, 1, LV_GRID_ALIGN_STRETCH, j + 1, 1);
    }
  }

  // Callbacks moved to interface_t to avoid calling concurrently from different
  // threads.

  // machine_interface_t *mach = interface->machine;
  // machine_interface_add_pos_changed_cb(mach, mp, pos_updated_callback);
  // machine_interface_add_pos_changed_cb(mach, mp,
  // interface_machine_state_updated_cb);
  // machine_interface_add_wcs_changed_cb(mach, mp, wcs_updated_callback);
  // interface_register_state_change_cb(interface,
  // interface_machine_state_updated_cb, mp);

  // Initial update
  wcs_updated_callback(interface->machine, mp);
  machine_position_wcs_coords_undefined(mp);  // all undefined to begin with.

  interface_add_state_changed_cb(interface, mp,
                                 interface_machine_state_updated_cb);
...
void machine_position_wcs_coords_undefined(machine_position_wcs_t *mp) {
  ...
  for (size_t i = 0; i < mp->num_coord_systems; i++) {
    for (size_t j = 0; j < mp->num_coords; j++) {
      ...
      lv_label_set_text_fmt(mp->coord_val_labels[i][j], "%.*s.??",
                            mp->digits - 2, "?????????????");
    }
  }
}

void machine_position_wcs_set_coord(machine_position_wcs_t *mp, size_t ax,
                                    float v, const char *coord_system) {
  ...
  // if we didn't find it, it's probably because we are initializing, so it's
  // ok.
  if (isnan(v)) {
...
void machine_position_wcs_align_to(machine_position_wcs_t *mp, lv_obj_t *target,
