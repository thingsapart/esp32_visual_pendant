...
#define SELECTED_AXIS_COLOR \
  lv_color_hex(0x9C27B0)  // Purple (Material Design Purple 500)
#define WCS_PREFIX "G"

/** Private */

static void lv_axis_position_display_constructor(const lv_obj_class_t *class_p,
                                                 lv_obj_t *obj);
static void lv_axis_position_display_destructor(const lv_obj_class_t *class_p,
                                                lv_obj_t *obj);
static void axis_label_event_cb(lv_event_t *e);
static void update_label_text(lv_obj_t *label, const char *symbol, float value,
                              const char *prefix);
static lv_obj_t *lv_axis_position_display_get_child(lv_obj_t *obj,
                                                    lv_part_t part);
static void lv_axis_position_display_event(const lv_obj_class_t *class_p,
                                           lv_event_t *e);
...
static void lv_axis_position_display_constructor(const lv_obj_class_t *class_p,
  ...
  // Temporary pointers to capture the created objects within the view macro
  // scope
  lv_obj_t *_axis, *_pos, *_dpos, *_apos, *_adpos_cont;

  view(obj, __text_color(lv_color_white()), __pad_row(5), __pad_column(5),
  ...
  _layout_h(_adpos_cont, LV_FLEX_ALIGN_CENTER, _flex(_dpos, 1),
            _flex(_apos, 1));
  lv_obj_set_style_pad_column(_adpos_cont, 5, 0);

  _layout_grid(
      obj, _cols(LV_GRID_CONTENT, _fr(1)), _rows(LV_GRID_CONTENT, _px(30)),

      _cell(disp->axis_label, 0, 0,
            _cell_opts(.col_align = LV_GRID_ALIGN_STRETCH,
                       .row_align = LV_GRID_ALIGN_CENTER)),
      _cell(disp->pos_label, 1, 0,
            _cell_opts(.col_align = LV_GRID_ALIGN_STRETCH,
                       .row_align = LV_GRID_ALIGN_CENTER)),
      _cell(
          disp->adpos_cont, 0, 1,
          _cell_opts(.col_span = 2, .col_align = LV_GRID_ALIGN_STRETCH,
                     .row_align = LV_GRID_ALIGN_STRETCH))  // Span 2 cols,
                                                           // stretch container
  );

  __flag(disp->axis_label, LV_OBJ_FLAG_CLICKABLE, true);
  __add_event_cb(disp->axis_label, axis_label_event_cb, LV_EVENT_CLICKED,
                 obj);  // Pass widget obj as user data
...
static void axis_label_event_cb(lv_event_t *e) {
  ...
  if (code == LV_EVENT_CLICKED && obj) {
    ...
    lv_color_t new_color =
        disp->selected ? SELECTED_AXIS_COLOR : INITIAL_AXIS_COLOR;
...
static void refresh_part_styles(lv_obj_t *obj) {
  ...
  if (disp->axis_label) {
    ...
    lv_text_align_t align =
        lv_obj_get_style_text_align(obj, LV_PART_AXIS_LABEL);
    // Add others as needed (letter_space, line_space, decor, opa)
...
static void update_label_text(lv_obj_t *label, const char *symbol, float value,
  ...
  char buf[32];
  if (prefix && symbol) {
    snprintf(buf, sizeof(buf), "%s%s %.*f", prefix, symbol,
             LV_AXIS_DEFAULT_POS_PRECISION, value);
  } else if (symbol) {
    snprintf(buf, sizeof(buf), "%s %.*f", symbol, LV_AXIS_DEFAULT_POS_PRECISION,
             value);
  } else if (prefix) {
    snprintf(buf, sizeof(buf), "%s%.*f", prefix, LV_AXIS_DEFAULT_POS_PRECISION,
             value);
  } else {
