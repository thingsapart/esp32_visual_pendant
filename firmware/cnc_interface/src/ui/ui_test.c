...
void test_ui(lv_obj_t *screen) {
  ...
  // --- Flex Example ---
  CONT("flex_container", screen, _size(obj, 300, 200);
       _align(obj, LV_ALIGN_TOP_MID); _pad_top(obj, 20, LV_PART_MAIN);
       _layout(obj, LV_LAYOUT_FLEX); _flex_row_wrap(obj);  // Enable wrapping
  ...
  // --- Grid Example ---
  CONT("grid_container", screen, _size(obj, 300, 200);
       _align(obj, LV_ALIGN_BOTTOM_MID); _pad_bottom(obj, 50, LV_PART_MAIN);
  ...
  LABEL("grid_label_1", grid_cont, _label_text(obj, "Cell 1");
  ...
  BTN("grid_btn_1", grid_cont,
      ...
      lv_obj_t *btn_label = lv_label_create(obj);
      _label_text(btn_label, "Button"); lv_obj_center(btn_label););

  IMG("grid_img_1", grid_cont,
      // Assuming you have an image asset declared elsewhere (e.g.,
      // LV_IMG_DECLARE(my_image)) _img_src(obj, &my_image); // You would set
      // your image source here.
      lv_img_set_src(obj,
                     LV_SYMBOL_OK);  // Use a built-in symbol as a placeholder
