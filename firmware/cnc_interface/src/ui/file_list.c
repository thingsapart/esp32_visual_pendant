...
static char *path_capitalize(const char *path, char *buffer,
                             size_t buffer_size) {
...
static void files_changed_cb(machine_interface_t *self, void *user_data,
...
file_list_t *file_list_create(lv_obj_t *parent, const char *path,
  ...
  fl->path = path;  // Assume path is a string literal or a dynamically
                    // allocated, persistent string
  fl->machine = machine;
...
void file_list_refresh(file_list_t *fl) {
  if (!fl) return;
  fl->machine->list_files(fl->machine,
                          fl->path);  // list files and trigger callback.
}
...
void file_list_show_files(file_list_t *fl, char **files) {
  ...
  lv_obj_t *lbl = lv_list_add_text(
      fl->list, path_capitalize(fl->path, path_buf, sizeof(path_buf)));
  if (!lbl) {
  ...
  while (files[i] != NULL) {
    ...
    lv_obj_add_event_cb(btn, btn_click_event_handler, LV_EVENT_CLICKED,
                        fl);  // Pass fl as user_data
    i++;
  }
}
...
void file_list_init_empty(file_list_t *fl) {
  ...
  lv_obj_t *btn =
      lv_list_add_btn(fl->list, LV_SYMBOL_REFRESH, "Waiting for connection...");
  if (!btn) {
