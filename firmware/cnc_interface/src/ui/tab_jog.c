...
static const feed_t feeds[] = {
    {"100%", 1.0f}, {"25%", 0.25f}, {"10%", 0.1f}, {"1%", 0.01f}};
static const size_t num_feeds = sizeof(feeds) / sizeof(feeds[0]);

const char *axes_options[] = {"X", "Y", "Z", "Off"};
static const size_t num_axes_options =
    sizeof(axes_options) / sizeof(axes_options[0]);
...
static const char *workspaces[] = {"G54", "G55",   "G56",   "G57",  "G58",
...
static const size_t num_coord_systems =
    sizeof(coord_systems) / sizeof(coord_systems[0]);
...
void machine_curr_move_axis_changed(machine_interface_t *machine,
                                    void *user_data) {
  jog_dial_t *jd = (jog_dial_t *)user_data;

  if (jd) {
    jog_dial_set_axis_vis(jd, machine->current_move_axis);
  }
}
...
static void axis_clicked_event_handler(lv_event_t *e) {
  ...
  machine_interface_set_current_move_axis(jd->interface->machine,
                                          get_axis_from_string(txt));
...
static void feed_changed_event_handler(lv_event_t *e) {
  ...
  if (jd) {
    ...
    // lv_obj_align_to(jd->feed_label, jd->feed_slider, LV_ALIGN_OUT_LEFT_MID,
    // -20, 0);
  }
}
static void jog_dial_value_changed_event_handler(lv_event_t *e) {
  ...
  if (jd) {
    ...
    // diff = val - jd->prev;
    // jd->interface.machine.move(jd->axis, jd->feed, diff); // Placeholder:
    // Call your interface
  }
}
...
jog_dial_t *jog_dial_create(lv_obj_t *parent, interface_t *interface) {
  ...
  jd->feed = 1.0f;
  jd->axis_id =
      get_axis_id(machine_interface_get_current_move_axis(interface->machine));
  ...
  _flex_align(cnt, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
  ...
  lv_btnmatrix_set_btn_ctrl(
      jd->axis_btns, num_axes_options - 1,
      LV_BTNMATRIX_CTRL_CHECKED);  // removed default checkmark
  ...
  lv_obj_add_event_cb(jd->axis_btns, axis_clicked_event_handler,
                      LV_EVENT_VALUE_CHANGED, jd);
  ...
  lv_obj_add_event_cb(jd->arc, jog_dial_value_changed_event_handler,
                      LV_EVENT_VALUE_CHANGED, jd);
  ...
  lv_obj_add_event_cb(jd->feed_slider, feed_changed_event_handler,
                      LV_EVENT_VALUE_CHANGED, jd);
  ...
  _flag(jd->feed_label, LV_OBJ_FLAG_IGNORE_LAYOUT, true);
  lv_obj_align_to(jd->feed_label, jd->feed_slider, LV_ALIGN_OUT_LEFT_MID, -5,
                  0);

  // Position Labels
  jd->position =
      machine_position_wcs_create(jd->arc, axes, num_axes, interface, 6,
                                  coord_systems, num_coord_systems, 60);
  if (!jd->position) {
  ...
  machine_interface_add_current_move_axis_changed_cb(
      interface->machine, jd, machine_curr_move_axis_changed);
...
axis_t jog_dial_next_axis(jog_dial_t *jd) {
  ...
  // lv_btnmatrix_set_btn_ctrl(interface->machine->axis_btns, i,
  // LV_BTNMATRIX_CTRL_CHECKED); jog_dial_set_axis_vis(jd, (axis_t)i);
  return next;
}
...
// --- TabJog Functions ---
tab_jog_t *tab_jog_create(lv_obj_t *tabv, interface_t *interface,
                          lv_obj_t *tab) {
  ...
  // tj->jog_slider = jog_slider_create(tab, interface); // If you implement
  // JogSlider
