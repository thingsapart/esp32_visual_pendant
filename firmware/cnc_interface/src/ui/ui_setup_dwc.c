...
static void create_ssid_modal(interface_t* interface, const char* ssids) {
  ...
  lv_obj_add_event_cb(dd, ssid_list_event_handler, LV_EVENT_VALUE_CHANGED,
                      interface);
  lv_obj_center(mbox);
}
...
static void ssid_list_event_handler(lv_event_t* e) {
  ...
  if (code == LV_EVENT_VALUE_CHANGED) {
    ...
    strncpy(interface->setup_settings.ssid, buf,
            sizeof(interface->setup_settings.ssid) - 1);
...
static void create_password_modal(interface_t* interface) {
  ...
  lv_obj_add_event_cb(kb, password_entry_event_handler, LV_EVENT_ALL,
                      interface);
...
static void password_entry_event_handler(lv_event_t* e) {
  ...
  if (lv_keyboard_get_btn_text(kb, lv_keyboard_get_selected_btn(kb)) ==
      LV_SYMBOL_OK) {
    strncpy(interface->setup_settings.password, lv_textarea_get_text(ta),
            sizeof(interface->setup_settings.password) - 1);
    lv_msgbox_close(mbox);
    lv_obj_del(kb);
    ...
    bool connected =
        wifi_manager_connect(interface->setup_settings.ssid,
                             interface->setup_settings.password, 15000);
  ...
  } else if (lv_keyboard_get_btn_text(kb, lv_keyboard_get_selected_btn(kb)) ==
             LV_SYMBOL_CLOSE) {
    lv_msgbox_close(mbox);
    lv_obj_del(kb);
  }
...
static void host_entry_event_handler(lv_event_t* e) {
  ...
  if (lv_keyboard_get_btn_text(kb, lv_keyboard_get_selected_btn(kb)) ==
      LV_SYMBOL_OK) {
    strncpy(interface->setup_settings.host, lv_textarea_get_text(ta),
            sizeof(interface->setup_settings.host) - 1);
    lv_msgbox_close(mbox);
    lv_obj_del(kb);
    ...
    bool is_ip_format =
        (strspn(interface->setup_settings.host, "0123456789.") ==
         strlen(interface->setup_settings.host));

    lv_msgbox_add_text(connecting_mbox, is_ip_format
                                            ? "Testing host connection..."
                                            : "Resolving hostname...");
    lv_refr_now(NULL);

    if (!wifi_manager_resolve_host(interface->setup_settings.host, host_to_use,
                                   sizeof(host_to_use))) {
      lv_obj_del(connecting_mbox);
      char error_msg[128];
      snprintf(error_msg, sizeof(error_msg),
               "Error: Could not resolve hostname\n%s",
               interface->setup_settings.host);
    ...
    if (client) {
      ...
      read_len = dwc_http_get(client, "/rr_status?type=1", response_buffer,
                              sizeof(response_buffer), &status_code);
      if (read_len >= 0 &&
          (status_code == 200 ||
           status_code == 401)) {  // 401 is also ok, it means server is there
        host_ok = true;
      }
    ...
    if (host_ok) {
      ...
      char error_msg[256];
      char full_url[128];
      snprintf(full_url, sizeof(full_url), "http://%s/rr_status?type=1",
               host_to_use);
      if (!client) {
        snprintf(error_msg, sizeof(error_msg),
        ...
        if (is_ip_format) {
          ...
          snprintf(error_msg, sizeof(error_msg),
                   ...
                   interface->setup_settings.host, resolved_ip_str, full_url);
        }
      } else {  // HTTP error
        snprintf(error_msg, sizeof(error_msg),
                 "Error: Host responded with HTTP %d.\n- Check DWC is "
                 "running.\n\nURL: %s",
...
static void create_wifi_fail_modal(interface_t* interface) {
  ...
  lv_msgbox_add_text(mbox,
                     "Could not connect to the Wi-Fi network. Please check the "
  ...
  lv_obj_add_event_cb(btn1, wifi_fail_modal_event_handler, LV_EVENT_CLICKED,
                      interface);
  lv_obj_add_event_cb(btn2, wifi_fail_modal_event_handler, LV_EVENT_CLICKED,
                      interface);
  lv_obj_center(mbox);
}
...
static void create_wifi_total_fail_modal(interface_t* interface) {
  ...
  lv_msgbox_add_text(mbox,
                     "Failed to connect multiple times.\n\nPlease check your "
  ...
  lv_obj_add_event_cb(btn1, wifi_total_fail_modal_event_handler,
                      LV_EVENT_CLICKED, interface);
  lv_obj_add_event_cb(btn2, wifi_total_fail_modal_event_handler,
                      LV_EVENT_CLICKED, interface);
  lv_obj_center(mbox);
}
