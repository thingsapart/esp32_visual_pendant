...
lvgl_port_ppa_handle_t lvgl_port_ppa_create(const lvgl_port_ppa_cfg_t *cfg)
{
    ...
    ppa_ctx->buffer_size = ALIGN_UP(cfg->buffer_size, 64); // PPA requires 64-byte alignment
    ppa_ctx->buffer = (uint8_t *)heap_caps_aligned_calloc(64, ppa_ctx->buffer_size, sizeof(uint8_t), buffer_caps);
...
lvgl_port_ppa_handle_t lvgl_port_ppa_create(const lvgl_port_ppa_cfg_t *cfg) { return NULL; }
void lvgl_port_ppa_delete(lvgl_port_ppa_handle_t handle) {}
uint8_t *lvgl_port_ppa_get_output_buffer(lvgl_port_ppa_handle_t handle) { return NULL; }
esp_err_t lvgl_port_ppa_rotate(lvgl_port_ppa_handle_t handle, lvgl_port_ppa_disp_rotate_t *rotate_cfg) { return ESP_ERR_NOT_SUPPORTED; }
#endif
