...
typedef struct {
  ...
  struct {
    ...
    unsigned int sw_rotate : 1;   /*!< Use software rotation (slower) or PPA if
                                     available */
#if LVGL_VERSION_MAJOR >= 9
    unsigned int swap_bytes : 1; /*!< Swap bytes in RGB565 (16-bit) color format
                                    before send to LCD driver */
#endif
    unsigned int
        full_refresh : 1; /*!< 1: Always make the whole screen redrawn */
...
typedef struct {
  struct {
    ...
    unsigned int
        avoid_tearing : 1; /*!< 1: Use internal RGB buffers as a LVGL draw
                              buffers to avoid tearing effect, enabling this
                              option requires over two LCD buffers and may
...
typedef struct {
  struct {
    unsigned int
        avoid_tearing : 1; /*!< 1: Use internal MIPI-DSI buffers as a LVGL draw
                              buffers to avoid tearing effect, enabling this
                              option requires over two LCD buffers and may
                              reduce the frame rate */
  } flags;
