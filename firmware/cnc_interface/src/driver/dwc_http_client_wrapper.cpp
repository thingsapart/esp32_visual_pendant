...
int dwc_http_get(dwc_http_handle_t handle, const char* path, char* response_buffer, size_t buffer_len, int* status_code) {
    ...
    char full_url[256];
    snprintf(full_url, sizeof(full_url), "http://%s:%d%s", context->host, context->port, path);
    ...
    *status_code = httpCode;
    ...
    if (httpCode > 0) {
        ...
        if ((size_t)payload_len >= buffer_len) {
            LOGW(TAG, "Response body (len %d) is larger than buffer (len %d), truncating.", payload_len, buffer_len);
        ...
        LOGE(TAG, "HTTP GET failed, error: %s", http.errorToString(httpCode).c_str());
        http.end();
        return -1;
    }
}
...
int dwc_http_get(dwc_http_handle_t handle, const char* path, char* response_buffer, size_t buffer_len, int* status_code) {
    ...
    LOGD(TAG, "Making GET request to http://%s:%d%s", context->host, context->port, path);
