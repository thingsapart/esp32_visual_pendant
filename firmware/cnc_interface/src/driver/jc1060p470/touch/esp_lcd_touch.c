...
bool esp_lcd_touch_get_coordinates(esp_lcd_touch_handle_t tp, uint16_t *x,
  ...
  if (tp->config.process_coordinates != NULL) {
    tp->config.process_coordinates(tp, x, y, strength, point_num,
                                   max_point_num);
  }

  /* Software coordinates adjustment needed */
  bool sw_adj_needed =
      ((tp->config.flags.mirror_x && (tp->set_mirror_x == NULL)) ||
...
esp_err_t esp_lcd_touch_register_interrupt_callback(
  ...
  if (callback != NULL) {
    ...
    /* ISR service can be installed from user before, then it returns invalid
     * state */
    if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
      LOGE(TAG, "GPIO ISR install failed");
      return ret;
    }
