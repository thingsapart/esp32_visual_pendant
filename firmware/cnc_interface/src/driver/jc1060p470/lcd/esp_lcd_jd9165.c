...
static esp_err_t panel_jd9165_invert_color(esp_lcd_panel_t *panel,
                                           bool invert_color_data);
static esp_err_t panel_jd9165_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                     bool mirror_y);
static esp_err_t panel_jd9165_disp_on_off(esp_lcd_panel_t *panel, bool on_off);

esp_err_t esp_lcd_new_panel_jd9165(
    const esp_lcd_panel_io_handle_t io,
    const esp_lcd_panel_dev_config_t *panel_dev_config,
    esp_lcd_panel_handle_t *ret_panel) {
  LOGI(TAG, "jd9165:1.0.1");
  ...
  LOGD(TAG, "new MIPI DPI panel @%p", panel_handle);
...
static esp_err_t panel_jd9165_init(esp_lcd_panel_t *panel) {
  ...
  uint8_t ID[3];
  ESP_RETURN_ON_ERROR(esp_lcd_panel_io_rx_param(io, 0x04, ID, 3), TAG,
                      "read ID failed");
  ...
  if (jd9165->init_cmds) {
    ...
    init_cmds_size =
        sizeof(vendor_specific_init_default) / sizeof(jd9165_lcd_init_cmd_t);
  }
  ...
  LOGD(TAG, "send init commands success");
