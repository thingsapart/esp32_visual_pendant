...
static const char *TAG = "jd9165_lcd";
esp_lcd_panel_handle_t panel_handle = NULL;
esp_lcd_panel_io_handle_t io_handle = NULL;

jd9165_lcd::jd9165_lcd(int8_t lcd_rst) { _lcd_rst = lcd_rst; }
...
void jd9165_lcd::begin() {
  ...
  esp_lcd_dpi_panel_config_t dpi_config =
      JD9165_1024_600_PANEL_60HZ_DPI_CONFIG(MIPI_DPI_PX_FORMAT);
  ...
  ESP_ERROR_CHECK(
      esp_lcd_new_panel_jd9165(io_handle, &panel_config, &panel_handle));
...
void jd9165_lcd::lcd_draw_bitmap(uint16_t x_start, uint16_t y_start,
...
void jd9165_lcd::draw16bitbergbbitmap(uint16_t x, uint16_t y, uint16_t w,
                                      uint16_t h, uint16_t *color_data) {
  uint16_t x_start = x;
  uint16_t y_start = y;
  uint16_t x_end = w + x;
  uint16_t y_end = h + y;
...
void jd9165_lcd::fillScreen(uint16_t color) {
  uint16_t *color_data =
      (uint16_t *)heap_caps_malloc(480 * 272 * 2, MALLOC_CAP_INTERNAL);
...
uint16_t jd9165_lcd::width() { return LCD_H_RES; }

uint16_t jd9165_lcd::height() { return LCD_V_RES; }
