...
typedef struct {
  ...
  uint8_t sub_type;  // binary_payload_sub_type_t: What the complete payload
                     // represents
  uint16_t seq_id;   // Sequence ID for the entire multi-fragment message
  uint32_t
      total_payload_size;  // Total size of the original binary payload in bytes
  ...
  uint32_t fragment_offset;  // Start offset of this fragment's data in the
                             // original payload
...
typedef union {
  ...
  // NOTE: binary_fragment_msg_t is NOT included here because of the flexible
  // array member. binary_fragment_msg_t binary_fragment;
} remote_msg_t;
