...
// Non-member operator+ declarations (needed before class for some
// compilers/orders)
...
class ArduinoString {
  ...
  static std::string integerToString(unsigned long long value, int base) {
    ...
    if (base == 10)
      return std::to_string(
          value);  // Use std::to_string for base 10 unsigned long long
    if (base == 16 || base == 8 ||
        base == 2) {  // Stringstream for HEX, OCT, BIN
      std::stringstream ss;
      if (base == 16)
        ss << std::hex << value;
      else if (base == 8)
 ...
 public:
  ...
  ArduinoString(const std::string& s)
      : _data(s) {}  // Allow construction from std::string
  ArduinoString(const ArduinoString& other) : _data(other._data) {}
  ArduinoString(char c) : _data(1, c) {}
  ArduinoString(unsigned char value, int base = 10)
      : _data(integerToString(value, base)) {}
  ArduinoString(int value, int base = 10)
      : _data(signedIntegerToString(value, base)) {}
  ArduinoString(unsigned int value, int base = 10)
      : _data(integerToString(value, base)) {}
  ArduinoString(long value, int base = 10)
      : _data(signedIntegerToString(value, base)) {}
  ArduinoString(unsigned long value, int base = 10)
      : _data(integerToString(value, base)) {}
  // Add long long / unsigned long long if needed for newer Arduino
  // cores/platforms
  ArduinoString(float value, unsigned int decimalPlaces = 2)
      : _data(floatToString(value, decimalPlaces)) {}
  ArduinoString(double value, unsigned int decimalPlaces = 2)
      : _data(floatToString(value, decimalPlaces)) {}

  // Allow explicit conversion from std::string
  explicit ArduinoString(std::string&& s)
      : _data(std::move(s)) {}  // Move constructor

  // --- Assignment ---
  ArduinoString& operator=(const ArduinoString& rhs) {
    _data = rhs._data;
    return *this;
  }
  ArduinoString& operator=(const char* cstr) {
    _data = (cstr ? cstr : "");
    return *this;
  }
  ArduinoString& operator=(const std::string& rhs) {
    _data = rhs;
    return *this;
  }
  ...
  ArduinoString& operator=(unsigned char value) {
    *this = ArduinoString(value);
    return *this;
  }
  ...
  ArduinoString& operator=(unsigned int value) {
    *this = ArduinoString(value);
    return *this;
  }
  ArduinoString& operator=(long value) {
    *this = ArduinoString(value);
    return *this;
  }
  ...
  ArduinoString& operator=(float value) {
    *this = ArduinoString(value);
    return *this;
  }
  ArduinoString& operator=(double value) {
    *this = ArduinoString(value);
    return *this;
  }

  // Move assignment
  ArduinoString& operator=(ArduinoString&& rhs) noexcept {
    _data = std::move(rhs._data);
    return *this;
  }
  ArduinoString& operator=(std::string&& rhs) noexcept {
    _data = std::move(rhs);
    return *this;
  }
  ...
  ArduinoString& operator+=(unsigned char val) {
    return (*this += ArduinoString(val));
  }
  ArduinoString& operator+=(int val) { return (*this += ArduinoString(val)); }
  ArduinoString& operator+=(unsigned int val) {
    return (*this += ArduinoString(val));
  }
  ArduinoString& operator+=(long val) { return (*this += ArduinoString(val)); }
  ArduinoString& operator+=(unsigned long val) {
    return (*this += ArduinoString(val));
  }
  ArduinoString& operator+=(float val) { return (*this += ArduinoString(val)); }
  ArduinoString& operator+=(double val) {
    return (*this += ArduinoString(val));
  }

  // concat() method variants - return true mirroring Arduino (failure unlikely
  // with std::string)
  bool concat(const ArduinoString& s) {
    _data += s._data;
    return true;
  }
  ...
  unsigned int length() const {
    return static_cast<unsigned int>(_data.length());
  }
  ...
  char charAt(unsigned int index) const {
    return (index < length()) ? _data[index] : '\0';
  }
  void setCharAt(unsigned int index, char c) {
    if (index < length()) _data[index] = c;
  }
  ...
  void getBytes(unsigned char* buf, unsigned int bufsize,
    ...
    // Number of chars to copy: minimum of remaining chars and (bufsize-1 for
    // null term)
  ...
  int compareTo(const char* cstr) const {
    return _data.compare(cstr ? cstr : "");
  }
  ...
  bool equalsIgnoreCase(const char* cstr) const {
    if (!cstr)
      return _data.empty();  // Match Arduino: s.equalsIgnoreCase(NULL) is true
                             // iff s is empty
    size_t cstr_len = std::strlen(cstr);
    if (length() != cstr_len) return false;
    return std::equal(_data.begin(), _data.end(), cstr, [](char a, char b) {
      return ::tolower(static_cast<unsigned char>(a)) ==
             ::tolower(static_cast<unsigned char>(b));
    });
  }
  ...
  // --- Search ---
  // Note: std::string::find returns std::string::npos on failure, Arduino
  // returns -1
  ...
  int lastIndexOf(char ch, int fromIndex = -1) const {
    size_t startPos =
        (fromIndex < 0 || static_cast<unsigned int>(fromIndex) >= length())
  ...
  ArduinoString substring(unsigned int beginIndex) const {
    if (beginIndex >= length())
      return ArduinoString();  // Return empty string if out of bounds
  ...
  ArduinoString substring(unsigned int beginIndex,
                          unsigned int endIndex) const {
    if (beginIndex >= length())
      return ArduinoString();  // Empty if start out of bounds
    if (beginIndex >= endIndex)
      return ArduinoString();  // Empty if end <= start
  ...
  void replace(const ArduinoString& find, const ArduinoString& replaceWith) {
    ...
    size_t start_pos = 0;
    while ((start_pos = _data.find(find._data, start_pos)) !=
           std::string::npos) {
  ...
  void trim() {
    ...
    auto first = std::find_if_not(_data.begin(), _data.end(),
                                  [](unsigned char c) { return ::isspace(c); });
    // Find the last non-whitespace character (searching from the end)
    auto last =
        std::find_if_not(_data.rbegin(), _data.rend(), [](unsigned char c) {
          return ::isspace(c);
        }).base();
  ...
  // Arduino's invalidate() is meant to free buffer; std::string::clear()
  // removes content but doesn't guarantee immediate memory release.
  // std::string::shrink_to_fit() is closer.
  void invalidate() {
    _data.clear();
  ...
  long toInt() const {
    try {
      ...
      // Arduino String allows trailing non-numeric chars, check if *any*
      // conversion happened
      return (endptr == _data.c_str()) ? 0 : val;
    } catch (...) {  // Catch potential exceptions from std::strtol if used
                     // differently, though less likely
      return 0;
    }
  }
  float toFloat() const {
  ...
  double toDouble()
      const {  // Add double if needed (often same as float on Arduino)
    try {
      char* endptr;
  ...
  friend ArduinoString operator+(const ArduinoString& lhs,
                                 const ArduinoString& rhs) {
  ...
  // explicit operator std::string() const { return _data; } // Explicit
  // prevents unwanted implicit conversions
  std::string toString() const {
    return _data;
  }  // Safer alternative: explicit method
};

// Define symmetric comparison operators for const char* on LHS
inline bool operator==(const char* lhs, const ArduinoString& rhs) {
  return rhs == lhs;
}
inline bool operator!=(const char* lhs, const ArduinoString& rhs) {
  return rhs != lhs;
