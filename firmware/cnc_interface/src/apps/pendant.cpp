...
static TaskHandle_t lv_tick_task_handle = NULL;
...
void setup() {
  ...
  if (create_res == pdPASS) {
    ...
    task_registry_register_handle(lvgl_task_handle, "lvgl_task");
  } else {
  ...
  // Create tick task if USE_TICK_HOOK is not set.
    xTaskCreatePinnedToCore(
      ...
      &lv_tick_task_handle,   // Task handle (optional, can be used to control the
          // task)
      0);
  LOGI(TAG, "DONE: Tick Task Pinned...\n");
    task_registry_register_handle(lv_tick_task_handle, "lv_tick_task");
  ...
  if (!abort && machine_send_task_run(
    ...
    LOGI(TAG, "DONE\n");
    task_registry_register_handle(machine_send_task_handle, "MachineSendTask");
  } else {
