...
/*
 * This machine interface uses multiple "channels" to connect to the _same_
 * machine, _not to multiple machines_. That is, it has only a single state for
 * a single machine that can be maintained via multiple channels (say,
 * wirelessly or wired via serial). Generally, it's meant to be used with one of
 * these channels being active but should work with multiple channels at the
