...
static bool _machine_rrf_read_response(machine_rrf_t *self, char *buffer,
                                       size_t buffer_size);
...
static void _machine_rrf_continuous_move(machine_interface_t *self,
                                         const char axis, float feed,
                                         int direction);

// JSON parsing helpers (private)
static bool _machine_rrf_parse_json_response(machine_rrf_t *self,
                                             const char *json_response);
static bool _machine_rrf_parse_m409_response(machine_rrf_t *self,
                                             cJSON *json_obj);
static bool _machine_rrf_parse_move_axes_brief(machine_rrf_t *self,
                                               cJSON *axes_array);
static bool _machine_rrf_parse_move_axes_ext(machine_rrf_t *self,
                                             cJSON *axes_array);
static bool _machine_rrf_parse_move_axes(machine_rrf_t *self,
                                         cJSON *axes_array);
static bool _machine_rrf_parse_globals(machine_rrf_t *self, cJSON *globals_obj);
static bool _machine_rrf_parse_m20_response(machine_rrf_t *self,
                                            cJSON *json_obj);
...
static bool _machine_rrf_read_response(machine_rrf_t *self, char *buffer,
  ...
  /*
  //_d(0, "Reading serial...\n");
  size_t len = serial_read_line_buf(self->uart, buffer, buffer_size,
  READ_TIMEOUT_MS); if (len > 0) { buffer[len] = '\0'; // Null-terminate the
  string
...
static void _machine_rrf_proc_machine_state(machine_rrf_t *self,
                                            const char *cmd) {
  ...
  if (_machine_rrf_read_response(self, response_buffer,
  sizeof(response_buffer))) { _machine_rrf_parse_json_response(self,
  response_buffer); if (!self->connected) { self->connected = true;
          self->message_box_last_dismissed_seq = -99999;
          machine_interface_connected_updated(&self->base);
...
static void _machine_rrf_update_machine_state(machine_interface_t *self,
  ...
  if (poll_state & MACHINE_POSITION) {
    ...
    if (rrf_self->input_sel && *rrf_self->input_sel != '\0') {
      char cmd2[64];
      snprintf(cmd2, sizeof(cmd2), "M409 K\"%s\" F\"d6,f\"",
               rrf_self->input_sel);
      _machine_rrf_proc_machine_state(rrf_self, cmd2);
    }
    _machine_rrf_proc_machine_state(rrf_self,
                                    "M409 K\"move.currentMove\" F\"f\"");
    _machine_rrf_proc_machine_state(rrf_self,
                                    "M409 K\"move.speedFactor\" F\"f\"");
  }
  if (poll_state & MACHINE_POSITION_EXT) {
    _machine_rrf_proc_machine_state(rrf_self,
                                    "M409 K\"move.axes[]\" F\"d5,v\"");
  }
...
static void _machine_rrf_continuous_move(machine_interface_t *self,
  ...
  snprintf(cmd, sizeof(cmd),
           "M98 P\"pendant-continuous-run.g\" A\"%c\" F%u D%u", axis,
           (unsigned int)feed, direction);
...
static bool _machine_rrf_parse_move_axes_brief(machine_rrf_t *self,
  ...
  cJSON_ArrayForEach(axis, axes_array) {
    cJSON *machine_pos_json =
        cJSON_GetObjectItemCaseSensitive(axis, "machinePosition");
    cJSON *wcs_pos_json =
        cJSON_GetObjectItemCaseSensitive(axis, "userPosition");
...
static bool _machine_rrf_parse_move_axes_ext(machine_rrf_t *self,
  ...
  cJSON_ArrayForEach(axis, axes_array) {
    ...
    if (cJSON_IsString(name_json) && cJSON_IsBool(homed_json) &&
      ...
      int i = machine_rrf_axis_idx(
          name[0]);         // Assuming single-character axis names
...
static bool _machine_rrf_parse_globals(machine_rrf_t *self,
                                       cJSON *globals_obj) {
...
void _free_modal(machine_interface_t *self, int modal_id) {
  if (self->message_box && (self->message_box->seq == modal_id)) {
    ...
    if (self->message_box->title) {
      free(self->message_box->title);
    }
    if (self->message_box->text) {
      free(self->message_box->text);
    }
    if (self->message_box->choices) {
      free(self->message_box->choices);
    }
...
static bool _machine_rrf_parse_m20_response(machine_rrf_t *self,
  ...
  while (dir_len > 0 &&
         (dir_str[dir_len - 1] == '/' || dir_str[dir_len - 1] == '"')) {
    dir_len--;
  }
  ...
  for (int j = 0; j < MAX_FILE_LISTS; j++) {
    if (self->base.filelists[j].fdir &&
        strcmp(self->base.filelists[j].fdir, clean_dir) == 0) {
      filelist_index = j;
      break;
    }
  }
  if (filelist_index == -1) {  // Not found, find an empty slot
    for (int j = 0; j < MAX_FILE_LISTS; j++) {
      if (self->base.filelists[j].fdir == NULL) {
        filelist_index = j;
        self->base.filelists[j].fdir =
            strdup(clean_dir);  // Store the directory
        break;
      }
    }
  }
  ...
  if (filelist_index != -1) {
    ...
    if (self->base.filelists[filelist_index].files) {
      for (int j = 0; self->base.filelists[filelist_index].files[j] != NULL;
           j++) {
...
static bool _machine_rrf_parse_m409_response(machine_rrf_t *self,
  ...
  if (strcmp(key, "move.axes") == 0 || strcmp(key, "move.axes[]") == 0) {
    ...
    if (len > self->base.num_spindles) {
      self->base.spindles =
          realloc(self->base.spindles, len * sizeof(spindle_t));
    ...
    if (fabsf(self->base.feed - old_feed) > 0.0001 ||
        fabsf(self->base.feed_req - old_feed_req) > 0.0001) {
    ...
    if (cJSON_IsNull(result_json)) {
      return true;
    }  // ? But suppresses errors when not moving.
    ...
    if (cJSON_HasObjectItem(result_json, "seq") &&
        (cJSON_HasObjectItem(result_json, "title") ||
         cJSON_HasObjectItem(result_json, "message"))) {
      ...
      LOGI(TAG, "base.msg_box: %p, base seq: %d, last_seq: %d",
           self->base.message_box,
           self->base.message_box ? self->base.message_box->seq : -1,
           self->message_box_last_dismissed_seq);

      if (!self->base.message_box ||
          (self->base.message_box->seq != seq &&
           seq > self->message_box_last_dismissed_seq)) {
    ...
    if (cJSON_IsNumber(result_json)) {
      ...
      snprintf(input_sel_str, sizeof(input_sel_str), "inputs[%d].axesRelative",
               self->input_idx);
    ...
    cJSON_ArrayForEach(name_json, result_json) {
      if (cJSON_IsString(name_json)) {
        ...
        if (strcmp(name, "Aux") == 0) {
          self->input_idx = i;
          char input_sel_str[64];
          snprintf(input_sel_str, sizeof(input_sel_str),
                   "inputs[%d].axesRelative", self->input_idx);
          self->input_sel = strdup(input_sel_str);  // Allocate and copy
        }
      }
      ++i;
...
static bool _machine_rrf_parse_json_response(machine_rrf_t *self,
  ...
  } else if (cJSON_GetObjectItemCaseSensitive(root, "key") ||
  ...
  if (!succ) {
    _df(2, "Failed to parse JSON response: %s", json_response);
  }
...
machine_rrf_t *machine_rrf_init(machine_rrf_t *self, int rrf_serial_num,
  ...
  self->base.send_gcode =
      machine_interface_send_gcode;  // Use the generic wrapper
...
void machine_rrf_task_loop_iter(machine_rrf_t *self) {
  // Delegate to the base class implementation, which will call our overridden
  // methods
...
static void serial_line_received_callback(serial_handle_t handle,
  ...
  if (task_event_queue == NULL) {
    ...
    LOGV(TAG, "Data ready for uart %p, task queue %p.",
         queue_serial_map[found].serial, task_event_queue);
  }
...
bool machine_rrf_setup_response_processing_task(machine_rrf_t *self,
  ...
  if (!serial_register_line_callback(self->uart,
  ...
  for (size_t i = 0; i < MAX_SERIAL_PROC_TASKS; ++i) {
    ...
    if (i == MAX_SERIAL_PROC_TASKS - 1) {
      LOGE(TAG,
           "Number of serial processing tasks exceeds MAX_SERIAL_PROC_TASKS "
           "(%d)",
           MAX_SERIAL_PROC_TASKS);
      return false;
    }
  }
