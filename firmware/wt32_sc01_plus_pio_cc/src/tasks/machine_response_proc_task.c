...
#define MAX_PROCESSING_TASKS 2
    ...
    static bool ring_buffer_add_line(ring_buffer_t *rb, const char *line, size_t len)
    {
        ...
        while (true)
        {
            ...
            if (write_ptr == NULL && required_len < rb->buffer_write_offset)
            { // Check if there's space *before* the current write offset
                ...
                if (required_len <= oldest_data_start || oldest_data_start == 0)
                { // Allow if fits before oldest or if oldest is also at 0 (buffer effectively linear at this point)
    ...
    static bool ring_buffer_get_line(ring_buffer_t *rb, char *out_buffer, size_t max_len, size_t *out_len)
    {
        if (!rb || !out_buffer || max_len == 0 || !out_len)
            return false;
    ...
    )
    {
...
#ifdef ESP32_HW
        // TODO: should consider moving this to class that calls if intent was to pass between threads. This makes it clearer that this code doesn't own the queue actually but the higher level processing does.
        *queue = xQueueCreate(QUEUE_LENGTH, sizeof(uint8_t));
        ...
        BaseType_t task_created = xTaskCreatePinnedToCore(
            ...
            pinned_core) == pdPASS;
#else
        bool task_created = thrd_create(task_handle, machine_response_proc_task, args) == thrd_success;
#endif
...
#ifdef ESP32_HW
            // TODO: if we move to higher up the queue creation we should also delete there vs here.
