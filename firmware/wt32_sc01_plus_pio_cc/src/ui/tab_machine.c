...
// would require a more complex data structure to represent the nested
// dictionaries.
static const char *materials[] = {"Aluminium",    "Hard Wood", "MDF",
                                  "Soft Wood",    "Acrylic",   "Hard Plastic",
                                  "Soft Plastic", NULL};
static const char *mills[] = {"1/8\"", "3mm",   "1/4\"", "6mm", "3/8\"",
                              "10mm",  "1/2\"", "12mm",  NULL};  // Example
...
static const char *mill_map[] = {  // an array of the above const arrays.
    mill_options, mill_options, mill_options, mill_options,
    mill_options, mill_options, mill_options};
...
static void set_wcs_btns_cb(lv_event_t *e) {
  ...
  machine_status_meter_t *msm =
      (machine_status_meter_t *)lv_event_get_user_data(e);
  if (btn && msm) {
    ...
    for (size_t i = 0; i < num_axes; i++) {
      if (btn == msm->position->axis_labels[i]) {
        ...
        char axis_char =
            msm->position->coords[i][0];  // Get the first character (e.g., 'X')
        msm->interface->machine->set_wcs_zero(
            msm->interface->machine, msm->interface->machine->wcs,
            (const char *)&axis_char);  // Pass as string.
        break;
      }
    }
  }
...
tab_machine_t *tab_machine_create(lv_obj_t *tabv, interface_t *interface,
                                  lv_obj_t *tab) {
...
void tab_machine_init_axis_float_btn(tab_machine_t *tm) {
  ...
  lv_label_set_text(
      label,
      axes_options[tm->interface->machine->current_move_axis]);  // set initial
                                                                 // axis.
  lv_obj_center(label);

  lv_obj_add_event_cb(tm->float_btn, axis_float_btn_cb, LV_EVENT_CLICKED,
                      tm);  // tm as user data
  machine_interface_add_current_move_axis_changed_cb(tm->interface->machine,
...
void tab_machine_init_tab_jobs(tab_machine_t *tm, lv_obj_t *tab) {
  return;
  tm->jobs_list =
      file_list_create(tab, "gcodes", tm->interface->machine, gcode_clicked);
  if (!tm->jobs_list) {
    // Handle error
    assert(false && "TODO");
  }
  _size(tm->jobs_list->list, lv_pct(100),
        lv_pct(100));  // Assuming file_list_t has a container
...
void tab_machine_init_tab_macros(tab_machine_t *tm, lv_obj_t *tab) {
  return;
  tm->macro_list =
      file_list_create(tab, "macros", tm->interface->machine, macro_clicked);
  if (!tm->macro_list) {
    // Handle error
  }
  _size(tm->macro_list->list, lv_pct(100),
        lv_pct(100));  // Assuming file_list_t has a container
  _style_local(tab, margin_all, LV_PART_MAIN, 0);
  _style_local(tab, pad_all, LV_PART_MAIN, 0);
...
machine_status_meter_t *machine_status_meter_create(lv_obj_t *parent,
                                                    ...
                                                    int max_feed) {
  machine_status_meter_t *msm =
      (machine_status_meter_t *)malloc(sizeof(machine_status_meter_t));
  if (!msm) {
  ...
  msm->spindle_max_rpm =
      (spindle_max_rpm / 100) * 100;       // Round down to nearest 100
  msm->max_feed = (max_feed / 100) * 100;  // Round down to nearest 100

  msm->container = mk_container(
      "msm:container", parent, _size(obj, lv_pct(100), lv_pct(100));
      _maximize_client_area(obj); _flex_flow(obj, LV_FLEX_FLOW_ROW);
      _pad_row(obj, 0); _pad_column(obj, 0);

      msm->left_side = mk_container(
          "msm:left", outer_obj, _flex_grow(obj, 1);
          _size(obj, lv_pct(60), lv_pct(100)); _bg_opa(obj, LV_OPA_TRANSP, _M);
          ...
          mk_label(NULL, outer_obj, _maximize_client_area(obj);
                   _label_text(obj, "Feed (mm/min):");
                   _size(obj, LV_SIZE_CONTENT, 16););

          msm->bar_feed = mk_bar(
              "msm:bar_feed", outer_obj,
              lv_bar_set_range(obj, 0, msm->max_feed);
              _maximize_client_area(obj); _bg_opa(obj, LV_OPA_TRANSP, _M);
          ...
          msm->scale_feed = mk_scale(
              ...
              _margins(obj, -12, 18, 0, 18); _pad_all(obj, 0, _M);
          ...
          // --- Spindle RPM Section ---
          mk_label(NULL, outer_obj, _label_text(obj, "Spindle (RPM):");
                   _maximize_client_area(obj);
                   _size(obj, LV_SIZE_CONTENT, 16););

          msm->spindle_rpm =
              mk_bar("msm:spindle_rpm", outer_obj, _maximize_client_area(obj);
                     lv_bar_set_range(obj, 0, msm->spindle_max_rpm);
                     _pads(obj, 5, 18, 5, 18); _size(obj, lv_pct(100), 18);
                     _bg_opa(obj, LV_OPA_TRANSP, _M);
                     lv_bar_set_value(obj, 5000, LV_ANIM_ON);  // example value.
              );
          msm->scale_spindle_rpm = mk_scale(
              NULL, outer_obj, _maximize_client_area(obj);
              _margins(obj, -12, 18, 0, 18); _pad_all(obj, 0, _M);
              _size(obj, lv_pct(100), 20);

              lv_scale_set_mode(obj, LV_SCALE_MODE_HORIZONTAL_BOTTOM);
              lv_scale_set_total_tick_count(obj,
                                            msm->spindle_max_rpm / 1000 + 1);
          ...
          mk_container(
              NULL, outer_obj, _size(obj, lv_pct(100), LV_SIZE_CONTENT);
              _flag(obj, LV_OBJ_FLAG_SCROLLABLE, false);
              _flex_flow(obj, LV_FLEX_FLOW_COLUMN); _maximize_client_area(obj);
              _scrollable(obj, false);

              mk_label(NULL, outer_obj, _label_text(obj, "Chip Load (x0.1mm)");
                       _maximize_client_area(obj);
                       _size(obj, LV_SIZE_CONTENT, 16););
              ...
              mk_container(
                  NULL, outer_obj, _maximize_client_area(obj);
                  _flex_grow(obj, 1); _scrollable(obj, false);
                  ...
                  msm->bar_cl =
                      mk_bar("msm:bar_spnd_cl", outer_obj,
                             lv_bar_set_range(obj, 0, 50);
                             _margin(obj, 0); _bg_opa(obj, LV_OPA_TRANSP, _M);
                             _pads(obj, 5, 18, 5, 18);
                             ...
                             _bar_indicator(
                                 obj, spnd_bar_cl, LV_OPA_COVER,
                                 lv_color_hex(0x00DDDD), lv_color_hex(0x00DD00),
                                 LV_GRAD_DIR_HOR, (int)(255 * 0.3f), 3););

                  msm->scale_spindle_chipload = mk_scale(
                      "msm:scl_spnd_cl", outer_obj, _maximize_client_area(obj);
                      _margins(obj, -12, 18, 0, 18); _pad_all(obj, 0, _M);
                      _size(obj, lv_pct(100), 20);
                      ...
                      _bar_indicator(obj, spnd_scale_cl, LV_OPA_COVER,
                                     lv_color_hex(0x00DDDD),
                                     lv_color_hex(0x00DD00), LV_GRAD_DIR_HOR,
                                     (int)(255 * 0.3f), 3););););

          // --- Material and End Mill Dropdowns ---
          mk_container(
              NULL, outer_obj, _bg_opa(obj, LV_OPA_TRANSP, _M);
              _maximize_client_area(obj); _pad_bottom(obj, 5, LV_STATE_DEFAULT);
              _flex_flow(obj, LV_FLEX_FLOW_ROW); _pad_column(obj, 5);
              _size(obj, lv_pct(100), LV_SIZE_CONTENT);

              msm->material_dd = mk_dropdown(
                  "msm:mat_dd", outer_obj,
                  lv_dropdown_set_options_static(
                      obj,
                      "Aluminium\nH.Wood\nMDF\nS.Wood\nAcrylic\nH.Plastic\nS."
                      "Plastic");
                  _maximize_client_area(obj); _flex_grow(obj, 1);
                  lv_obj_add_event_cb(obj, mat_dd_change,
                                      LV_EVENT_VALUE_CHANGED, msm););
              msm->mill_dd = mk_dropdown(
                  "msm:mill_dd", outer_obj,
                  lv_dropdown_set_options_static(obj, mill_options);
                  _maximize_client_area(obj); _flex_grow(obj, 1);
                  lv_obj_send_event(obj, LV_EVENT_VALUE_CHANGED, msm););
              msm->flute_dd = mk_dropdown(
                  "msm:flute_dd", outer_obj,
                  lv_dropdown_set_options_static(obj, "1F\n2F\n3F\n4F");
                  _maximize_client_area(obj); _flex_grow(obj, 1);););

      );

      msm->right_side = mk_container(
          NULL, outer_obj, _maximize_client_area(obj);
          _flex_flow(obj, LV_FLEX_FLOW_ROW_WRAP); _size(obj, 220, lv_pct(100));
          lv_obj_set_flex_align(obj, LV_FLEX_ALIGN_SPACE_EVENLY,
                                LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_START);
          _bg_opa(obj, LV_OPA_TRANSP, _M); _border_width(obj, 0, _M);
          _pad_right(obj, 10, _M); _pad_bottom(obj, 10, _M);
          _text_font(obj, &lv_font_montserrat_12, _M); _scrollable(obj, false);

          // --- Machine Coordinates ---
          static const char *default_coord_systems[] =
              {"Mach", "WCS"};  // Use static strings
          static const size_t num_default_coord_systems =
              sizeof(default_coord_systems) / sizeof(default_coord_systems[0]);

#if !HIDE_MACH_STATUS_METER_POS_WCS
          msm->position = machine_position_wcs_create(
              outer_obj, axes, num_axes, msm->interface, 6,
              default_coord_systems, num_default_coord_systems,
              40);  // exclude "Move"
          if (!msm->position) {
          ...
          // machine_position_wcs_align_to(msm->position, msm->right_side,
          // LV_ALIGN_TOP_MID, 0, 0);
          _margin_top(msm->position->container, 20, LV_STATE_DEFAULT);

          for (size_t i = 0; i < num_axes; i++) {
            mk_btn(
                "msm:wcs:x", outer_obj, msm->position->axis_labels[i] = obj;
                lv_obj_add_flag(obj, LV_OBJ_FLAG_FLEX_IN_NEW_TRACK);
                _maximize_client_area(obj); _size(obj, 150, 25);
                lv_obj_center(obj);
                lv_obj_add_event_cb(obj, set_wcs_btns_cb, LV_EVENT_CLICKED,
                                    msm);  // save msm as user data.

                mk_label(NULL, outer_obj, _maximize_client_area(obj);
                         _center(obj);

                         char buf[65]; snprintf(buf, 64, "%s Set WCS origin %s",
                                                LV_SYMBOL_REFRESH, axes[i]);
                         _label_text(obj, buf);););
          }
#endif
      ););
...
void machine_status_meter_destroy(machine_status_meter_t *msm) {
  ...
  if (msm->position) {
    machine_position_wcs_destroy(
        msm->position);  // Destroy the MachinePositionWCS object
  }
