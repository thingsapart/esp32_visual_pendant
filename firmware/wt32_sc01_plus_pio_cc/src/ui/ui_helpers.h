...
#define CONT(id, parent, ...) \
  CREATE_OBJECT(obj, id, parent, __VA_ARGS__)  // Container
...
#define TEXTAREA(id, parent, ...) \
  CREATE_OBJECT(textarea, id, parent, __VA_ARGS__)
#define CHECKBOX(id, parent, ...) \
  CREATE_OBJECT(checkbox, id, parent, __VA_ARGS__)
#define DROPDOWN(id, parent, ...) \
  CREATE_OBJECT(dropdown, id, parent, __VA_ARGS__)
...
#define mk_container(id, parent, ...) \
  CREATE_OBJECT(obj, id, parent, __VA_ARGS__)  // Container
#define mk_slider(id, parent, ...) \
  CREATE_OBJECT(slider, id, parent, __VA_ARGS__)
#define mk_switch(id, parent, ...) \
  CREATE_OBJECT(switch, id, parent, __VA_ARGS__)
#define mk_textarea(id, parent, ...) \
  CREATE_OBJECT(textarea, id, parent, __VA_ARGS__)
#define mk_checkbox(id, parent, ...) \
  CREATE_OBJECT(checkbox, id, parent, __VA_ARGS__)
#define mk_dropdown(id, parent, ...) \
  CREATE_OBJECT(dropdown, id, parent, __VA_ARGS__)
#define mk_roller(id, parent, ...) \
  CREATE_OBJECT(roller, id, parent, __VA_ARGS__)
...
#define _use_layout(obj, enabled) \
  _flag(obj, LV_OBJ_FLAG_IGNORE_LAYOUT, !enabled)
...
#define _style_local(obj, prop, part, val) \
  lv_obj_set_style_##prop(obj, val, part)
...
#define _border_color(obj, color, part) \
  lv_obj_set_style_border_color(obj, color, part)
#define _border_width(obj, width, part) \
...
#define _img_recolor(obj, color, part) \
  lv_obj_set_style_img_recolor(obj, color, part)
#define _img_recolor_opa(obj, opa, part) \
  lv_obj_set_style_img_recolor_opa(obj, opa, part)
#define _outline_width(obj, width, part) \
  lv_obj_set_style_outline_width(obj, width, part)
#define _outline_color(obj, color, part) \
  lv_obj_set_style_outline_color(obj, color, part)
#define _shadow_width(obj, width, part) \
  lv_obj_set_style_shadow_width(obj, width, part)
#define _shadow_color(obj, color, part) \
  lv_obj_set_style_shadow_color(obj, color, part)
#define _margin_all(obj, margin, part) \
  lv_obj_set_style_margin_all(obj, margin, part)
#define _margin_top(obj, margin, part) \
  lv_obj_set_style_margin_top(obj, margin, part)
#define _margin_bottom(obj, margin, part) \
  lv_obj_set_style_margin_bottom(obj, margin, part)
#define _margin_left(obj, margin, part) \
  lv_obj_set_style_margin_left(obj, margin, part)
#define _margin_right(obj, margin, part) \
  lv_obj_set_style_margin_right(obj, margin, part)
...
// Switch
#define _switch_state(obj, state, anim) \
  ((state) ? lv_switch_on(obj, anim) : lv_switch_off(obj, anim))

// Textarea
#define _textarea_text(obj, text) lv_textarea_set_text(obj, text)
#define _textarea_placeholder(obj, text) \
...
#define _checkbox_checked(obj, checked) \
  ((checked) ? lv_checkbox_set_checked(obj) : lv_checkbox_set_unchecked(obj))
...
// Roller
#define _roller_options(obj, options, mode) \
  lv_roller_set_options(obj, options, mode)
#define _roller_selected(obj, sel, anim) lv_roller_set_selected(obj, sel, anim)
#define _roller_visible_row_count(obj, cnt) \
  lv_roller_set_visible_row_count(obj, cnt)
...
#define self_size_valid_(obj) \
  lv_obj_is_layout_valid(obj)  // check if size is valid (layouting)
...
#define _flex_center_all(obj)                                  \
  _flex_align(obj, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, \
              LV_FLEX_ALIGN_CENTER)
#define _flex_justify_start(obj)                             \
  _flex_align(obj, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, \
              LV_FLEX_ALIGN_START)  // consistent naming.
#define _flex_justify_end(obj)                             \
  _flex_align(obj, LV_FLEX_ALIGN_END, LV_FLEX_ALIGN_START, \
              LV_FLEX_ALIGN_START)  // consistent naming.
#define _flex_space_between(obj)                                     \
  _flex_align(obj, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_START, \
              LV_FLEX_ALIGN_START)  // consistent naming.
#define _flex_space_around(obj)                                     \
  _flex_align(obj, LV_FLEX_ALIGN_SPACE_AROUND, LV_FLEX_ALIGN_START, \
              LV_FLEX_ALIGN_START)  // consistent naming.
#define _flex_space_evenly(obj)                                     \
  _flex_align(obj, LV_FLEX_ALIGN_SPACE_EVENLY, LV_FLEX_ALIGN_START, \
              LV_FLEX_ALIGN_START)  // consistent naming.
...
#define _rows(...) \
  { __VA_ARGS__ }
#define _grid_template(obj, cols, rows)                                    \
  do {                                                                     \
    lv_coord_t mcols[] = cols;                                             \
    lv_coord_t mrows[] = rows;                                             \
    mcols[(sizeof(mcols) / sizeof(mcols[0])) - 1] = LV_GRID_TEMPLATE_LAST; \
    mrows[(sizeof(mrows) / sizeof(mrows[0])) - 1] = LV_GRID_TEMPLATE_LAST; \
    lv_obj_set_grid_dsc_array(obj, mcols, mrows);                          \
  } while (0);
#define _grid_place_items(obj, col_place, row_place) \
  lv_obj_set_grid_align(obj, col_place, row_place)

// Set grid cell position and span for a *child* object.
#define _grid_cell(child, col_start, col_end, row_start, row_end)             \
  do {                                                                        \
    lv_obj_set_grid_cell(child, LV_GRID_ALIGN_START, col_start,               \
                         col_end - col_start, LV_GRID_ALIGN_START, row_start, \
                         row_end - row_start);                                \
  } while (0)
...
#define _grid_align_stretch_all(obj) \
  _grid_place_items(obj, LV_GRID_ALIGN_STRETCH, LV_GRID_ALIGN_STRETCH);
#define _grid_align_center_all(obj) \
  _grid_place_items(obj, LV_GRID_ALIGN_CENTER, LV_GRID_ALIGN_CENTER);
#define _grid_align_start_all(obj) \
  _grid_place_items(obj, LV_GRID_ALIGN_START, LV_GRID_ALIGN_START);
#define _grid_align_end_all(obj) \
  _grid_place_items(obj, LV_GRID_ALIGN_END, LV_GRID_ALIGN_END);
