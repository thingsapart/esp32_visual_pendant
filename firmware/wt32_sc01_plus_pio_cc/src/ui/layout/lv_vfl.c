...
void _vfl_do_linear_layout(lv_obj_t *parent, _vfl_linear_item_t items[],
                           size_t item_count, bool is_horizontal,
  ...
  for (size_t i = 0; i < item_count; ++i) {
    ...
    if (item->item_type == _VFL_ITEM_OBJ) {
      if (!item->obj) {
        LV_LOG_WARN("VFL Linear Layout: NULL object encountered at index %zu",
                    i);
        item->size_type = _VFL_SIZE_FIXED;
        item->value = 0;
        continue;
      }
      ...
      if (!lv_obj_is_valid(item->obj)) {
        LV_LOG_WARN(
            "VFL Linear Layout: Invalid object %p encountered at index %zu",
            (void *)item->obj, i);
        item->size_type = _VFL_SIZE_FIXED;
        item->value = 0;  // Treat as zero size
  ...
  lv_coord_t available_size =
      is_horizontal ? parent_inner_width : parent_inner_height;
  ...
  if (total_flex_factor > 0 && remaining_space > 0) {
    ...
    LV_LOG_INFO(
        "VFL Linear Layout: Not enough space. Flex items will have zero size.");
    remaining_space = 0;
  }

  // --- Pass 2 --- /* ... positioning ... */
  lv_coord_t current_pos = is_horizontal
  ...
  for (size_t i = 0; i < item_count; ++i) {
    ...
    if (item->item_type == _VFL_ITEM_OBJ) {
      ...
      if (!item->obj
          ...
          || !lv_obj_is_valid(item->obj)  // Re-check validity in case it
                                          // changed? Usually not needed.
#endif
      )
        continue;
      ...
      switch (item->size_type) { /* ... size assignment ... */
        case _VFL_SIZE_FIXED:
        case _VFL_SIZE_CONTENT:
          item_size = item->value;
          break;
        case _VFL_SIZE_FLEX:
          item_size = space_per_flex_unit * item->value;
          break;
      }
      if (item_size < 0) item_size = 0;
      lv_align_t item_align = (item->cross_align == _VFL_ALIGN_INHERIT)
                                  ? default_cross_align
                                  : item->cross_align;
      ...
      lv_coord_t space_value =
          (item->value == -1) ? default_space : item->value;
...
void _vfl_place_grid_items(lv_obj_t *parent, ...) {
  ...
  if (!parent) {
    LV_LOG_ERROR(
        "_vfl_place_grid_items: FATAL: Called with NULL parent pointer.");
  ...
  if (!lv_obj_is_valid(parent)) {
    LV_LOG_ERROR(
        "_vfl_place_grid_items: FATAL: Parent object %p provided to "
        "_layout_grid is invalid/deleted.",
        (void *)parent);
    // Consider assert(lv_obj_is_valid(parent));
    return;  // Cannot proceed if parent is invalid
  }
#else
  // Fallback if lv_obj_is_valid isn't available/used: Log that we're proceeding
  // without the check. LV_LOG_WARN("_vfl_place_grid_items: lv_obj_is_valid
  // check not available/enabled for parent.");
#endif
  ...
  while (1) {
    ...
    if (placement.obj == NULL) {
      LV_LOG_WARN(
          "_vfl_place_grid_items: Skipped grid item #%d because a NULL object "
          "pointer was provided in _grid_item().",
          item_index);
      item_index++;
      continue;
    }
    ...
    if (!lv_obj_is_valid(placement.obj)) {
      LV_LOG_WARN(
          "_vfl_place_grid_items: Skipped grid item #%d for object at %p "
          "because it's not a valid LVGL object.",
          item_index, (void *)placement.obj);
      item_index++;
      continue;
    }
#else
    // LV_LOG_WARN("_vfl_place_grid_items: lv_obj_is_valid check not
    // available/enabled for child %p.", (void*)placement.obj);
#endif
    ...
    if (lv_obj_get_parent(placement.obj) != parent) {
      // Parent validity checked at function start, should still be valid here
      // unless there's a severe concurrency issue or a bug deletes it
      // mid-function.
      lv_obj_set_parent(placement.obj, parent);
    }

    // Apply grid cell properties - Line ~187 in lv_vfl.c
    // If crash still occurs here, it implies an issue within LVGL's grid
    // handling for a valid parent and valid child.
    lv_obj_set_grid_cell(placement.obj, placement.h_align, placement.col,
                         placement.col_span, placement.v_align, placement.row,
                         placement.row_span);
...
void lv_vfl_set_grid_cell(lv_obj_t *obj, int32_t col, int32_t row,
                          __cell_args_t opt) {
  lv_obj_set_grid_cell(obj, opt.col_align, col,
                       opt.col_span == 0 ? 1 : opt.col_span, opt.row_align, row,
                       opt.row_span == 0 ? 1 : opt.row_span);
}