...
// --- For Linear Layout ---
typedef enum { _VFL_ITEM_OBJ, _VFL_ITEM_SPACE } _vfl_linear_item_type_t;
...
#define _VFL_GRID_ITEM_SENTINEL \
  ((_vfl_grid_item_placement_t){.obj = NULL})  // Internal
 ...
 * @param ALIGN Default cross-axis alignment (e.g., LV_ALIGN_TOP_MID,
 * LV_ALIGN_CENTER).
 ...
 *  );
 */
#define __layout_size(obj, w, w_across)                \
  lv_obj_set_size(obj, __layout_vert__ ? w : w_across, \
                  __layout_vert__ ? w_across : w);
#define _spacer(w)                                   \
  do {                                               \
    lv_obj_t *__spcr = lv_obj_create(_vfl_parent_v); \
    __layout_size(__spcr, w, 1);                     \
  } while (0);
#define _flex(obj, fr)                     \
  do {                                     \
    lv_obj_set_parent(obj, _vfl_parent_v); \
    lv_obj_set_flex_grow(obj, fr);         \
  } while (0);
#define _fixed(obj, sz)                      \
  do {                                       \
    lv_obj_set_parent(obj, _vfl_parent_v);   \
    __layout_size(obj, sz, LV_SIZE_CONTENT); \
  } while (0);
#define _content(obj)                              \
  do {                                             \
    lv_obj_set_parent(obj, _vfl_parent_v);         \
    __size(obj, LV_SIZE_CONTENT, LV_SIZE_CONTENT); \
  } while (0);
#define _sized(obj, w, h)                  \
  do {                                     \
    lv_obj_set_parent(obj, _vfl_parent_v); \
    __size(obj, w, h);                     \
  } while (0);
 ...
 *      _grid_item(.obj=label, .row=0, .col=1, .col_span=2,
 * .h_align=LV_GRID_ALIGN_CENTER), _grid_item(.obj=slider, .row=1, .col=0,
 * .col_span=3, .v_align=LV_GRID_ALIGN_STRETCH)
 *  );
 */
#define _layout_grid(PARENT, COLS_DEF, ROWS_DEF, ...)           \
  do {                                                          \
    ...
    if (!_vfl_grid_parent) {                                    \
      LV_LOG_WARN("_layout_grid: NULL parent");                 \
      break;                                                    \
    }                                                           \
    ...
    lv_obj_set_grid_dsc_array(_vfl_grid_parent, _vfl_grid_cols, \
                              _vfl_grid_rows);                  \
...
#define _cell_opts(...)                              \
  ((__cell_args_t){.col_align = LV_GRID_ALIGN_START, \
                   .row_align = LV_GRID_ALIGN_START, \
                   __VA_ARGS__})
#define _cell_4_(obj, col, row, opt) lv_vfl_set_grid_cell(obj, col, row, opt);
#define _cell_3_(obj, col, row)                            \
 ...
 */
#define _cols(...) \
  { __VA_ARGS__, LV_GRID_TEMPLATE_LAST }
...
/* Note: _content() macro already defined for linear layout is reused for grid
 * tracks */
