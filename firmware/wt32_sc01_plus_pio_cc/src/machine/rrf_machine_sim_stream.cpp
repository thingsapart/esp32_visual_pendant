...
RRFMachineSimStream::RRFMachineSimStream(int uart_num) : uart_num_(uart_num) {
  ...
  wcs_ = 0;  // Default to G54 (index 1, but stored as 0 internally?) - RRF
             // default is G54 which is index 1. Let's set to 1. M409 reports
             // 1-based index. G53 selects 0.
...
int RRFMachineSimStream::available() { return output_buffer_.length(); }
...
size_t RRFMachineSimStream::readBytes(char *buffer, size_t length) {
  ...
  if (len > 0) {
    memcpy(buffer, output_buffer_.data(),
           len);                   // Use memcpy for potentially binary data
    output_buffer_.erase(0, len);  // Erase the read bytes
  }
  return len;
}

size_t RRFMachineSimStream::readBytesUntil(char terminator, char *buffer,
                                           size_t length) {
  // _d(0, "BYTESUNTIL");
  ...
  if (n == std::string::npos) {  // Terminator not found
    len_to_read = std::min(output_buffer_.length(),
                           length);  // Read up to length or available data
    if (len_to_read > 0) {
      memcpy(buffer, output_buffer_.data(), len_to_read);
      output_buffer_.erase(0, len_to_read);
    }
    // Arduino Stream::readBytesUntil returns 0 if terminator not found within
    // length. This implementation reads data even if terminator is not found,
    // matching Serial.readBytesUntil behavior better? Let's stick to the
    // interface: return 0 if terminator not found within buffer length limit
    // 'length'. Check if terminator exists *at all* first.
    if (output_buffer_.find(terminator) == std::string::npos) {
      return 0;  // Mimic Arduino: Terminator not found, return 0 bytes read.
    }
    // If terminator exists *after* length, we should read up to length? No, the
    // spec says read *until* terminator. Let's re-read the Arduino spec. It
    // reads until terminator OR length is reached. If terminator is found: read
    // up to terminator or length, whichever comes first.
    ...
    len_to_read = std::min(
        n, length -
               1);  // Read up to terminator or length-1 to leave space for null
    if (len_to_read > 0) {
      memcpy(buffer, output_buffer_.data(), len_to_read);
    }
...
std::string RRFMachineSimStream::readStringUntil(const char terminator) {
  ...
  if (n == std::string::npos) {
    // Terminator not found. Arduino String would return the whole buffer
    // content and empty it. Let's mimic that. Return empty string if timeout
    // occurs (not applicable here) or buffer empty. If buffer not empty but no
    // terminator, Arduino waits for timeout. Simulating no timeout, return ""?
    // Or return current buffer? Let's return empty string for consistency with
    // timeout case.
    return "";
  }
...
size_t RRFMachineSimStream::write(uint8_t byte) {
  ...
  if (byte == '\n') {
    // Make a copy to process, in case process_gcode clears input_buffer_ or
    // yields
    std::string line_to_process = input_buffer_;
    input_buffer_.clear();  // Clear the input buffer immediately
    // Unlock mutex before calling process_gcode which might call write again
    // (via generate_response -> output_buffer_ -> read -> ...) or block for a
    // while. This requires process_gcode and its callees to be thread-safe
    // regarding internal state. Let's assume process_gcode is reasonably fast
    // and doesn't cause deadlock. Keep lock for now.
...
size_t RRFMachineSimStream::write(const uint8_t *buffer, size_t size) {
  ...
  while (current_pos < size) {
    const char *newline_pos = static_cast<const char *>(
        memchr(char_buffer + current_pos, '\n', size - current_pos));
    size_t chunk_len;
    ...
    if (found_newline) {
      ...
      // Consider unlocking mutex here if process_gcode is slow or might
      // re-enter write
      process_gcode(line_to_process.c_str());
    }
...
void RRFMachineSimStream::flush() {
  // Nothing to do here for the simulator (no actual hardware buffer to flush
  // TO) If flush means "wait until all written data is sent", it's also N/A
  // here.
}
...
void RRFMachineSimStream::process_last_command(std::string command_str) {
  // command_str is already a std::string, trim it if needed (process_gcode
  // trimming might be sufficient) trim(command_str); // Already trimmed in
  // process_gcode before tokenization
  ...
  if (command_str == "G28") {
    ...
    if (l20_found && wcs_index != -1) {
      ...
      if (!l20_found)
        LOGW(TAG, "G10 command requires L20 parameter for setting WCS");
      if (wcs_index == -1)
        LOGW(TAG, "G10 L20 command requires P parameter for WCS index");
    }
  } else if (command_str == "G1" || command_str == "G0") {
    float x = NAN, y = NAN,
          z = NAN;  // Use NAN to track which axes were specified
    ...
    if (!relative) {  // Absolute mode (G90)
      ...
      if (!std::isnan(x))
        pos_[0] =
            x - (wcs_ == 0
                     ? 0
                     : wcs_offsets_[wcs_ - 1][0]);  // Target user pos ->
                                                    // convert to machine pos
      if (!std::isnan(y))
        pos_[1] = y - (wcs_ == 0 ? 0 : wcs_offsets_[wcs_ - 1][1]);
      if (!std::isnan(z))
        pos_[2] = z - (wcs_ == 0 ? 0 : wcs_offsets_[wcs_ - 1][2]);
    ...
    _df(0,
        ">> G0/G1 %s (%s) => X:%.3f Y:%.3f Z:%.3f | Machine Pos: %.3f, %.3f, "
        "%.3f",
        command_str.c_str(), relative ? "Rel" : "Abs", std::isnan(x) ? NAN : x,
        std::isnan(y) ? NAN : y, std::isnan(z) ? NAN : z, pos_[0], pos_[1],
        pos_[2]);
  ...
  // Process potential chained commands (e.g., G90 G0 X10) - Original code
  // seemed to do this. This recursive call logic might be flawed if arguments
  // aren't commands. A better parser would handle multiple G/M codes on one
  // line properly. Let's disable the recursive part for now as it seems
  // fragile.
  /*
  for (size_t i = 0; i < current_args.size(); ++i) {
      // This logic assumes arguments might be commands, which is unusual for
  GCode.
...
// Generates "canned" RRF responses, example. Uses std::vector<std::string> for
// args.
void RRFMachineSimStream::generate_response(
    const char *command, const std::vector<std::string> &args) {
  ...
  if (strcmp(command, "M409") == 0) {
    if (!args.empty()) {
      ...
      if (!key.empty()) {
        if (key == "move.axes" || key == "move.axes[]") {
          ...
          int current_wcs_index =
              (wcs_ == 0) ? -1 : (wcs_ - 1);  // -1 if G53 active, else 0-8
          float user_x = pos_[0] + ((current_wcs_index >= 0)
                                        ? wcs_offsets_[current_wcs_index][0]
                                        : 0.0f);
          float user_y = pos_[1] + ((current_wcs_index >= 0)
                                        ? wcs_offsets_[current_wcs_index][1]
                                        : 0.0f);
          float user_z = pos_[2] + ((current_wcs_index >= 0)
                                        ? wcs_offsets_[current_wcs_index][2]
                                        : 0.0f);
          ...
          // Format the JSON response - Note: Using snprintf with many floats
          // can be tricky. Consider using a JSON library for robustness if
          // complexity increases.
          int written = snprintf(
              buffer, sizeof(buffer),
              "{\"key\":\"move.axes[]\",\"flags\":\"\",\"result\":[{"
              "\"acceleration\":900.0,\"babystep\":0,\"backlash\":0,"
              "\"current\":1450,\"drivers\":[\"0.0\"],\"homed\":%s,\"jerk\":"
              "300.0,\"letter\":\"X\",\"machinePosition\":%.3f,\"max\":200.00,"
              "\"maxProbed\":false,\"microstepping\":{\"interpolated\":true,"
              "\"value\":16},\"min\":0,\"minProbed\":false,\"percentCurrent\":"
              "100,\"percentStstCurrent\":100,\"reducedAcceleration\":900.0,"
              "\"speed\":5000.0,\"stepsPerMm\":800.00,\"userPosition\":%.3f,"
              "\"visible\":true,"
              "\"workplaceOffsets\":[%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%."
              "3f,%.3f]},{"  // WCS offsets X[0..9]
              "\"acceleration\":900.0,\"babystep\":0,\"backlash\":0,"
              "\"current\":1450,\"drivers\":[\"0.1\"],\"homed\":%s,\"jerk\":"
              "300.0,\"letter\":\"Y\",\"machinePosition\":%.3f,\"max\":160.00,"
              "\"maxProbed\":false,\"microstepping\":{\"interpolated\":true,"
              "\"value\":16},\"min\":0,\"minProbed\":false,\"percentCurrent\":"
              "100,\"percentStstCurrent\":100,\"reducedAcceleration\":900.0,"
              "\"speed\":5000.0,\"stepsPerMm\":800.00,\"userPosition\":%.3f,"
              "\"visible\":true,"
              "\"workplaceOffsets\":[%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%."
              "3f,%.3f]},{"  // WCS offsets Y[0..9]
              "\"acceleration\":100.0,\"babystep\":0,\"backlash\":0,"
              "\"current\":1450,\"drivers\":[\"0.2\"],\"homed\":%s,\"jerk\":30."
              "0,\"letter\":\"Z\",\"machinePosition\":%.3f,\"max\":70.00,"
              "\"maxProbed\":false,\"microstepping\":{\"interpolated\":true,"
              "\"value\":16},\"min\":-1.00,\"minProbed\":false,"
              "\"percentCurrent\":100,\"percentStstCurrent\":100,"
              "\"reducedAcceleration\":100.0,\"speed\":1000.0,\"stepsPerMm\":"
              "400.00,\"userPosition\":%.3f,\"visible\":true,"
              "\"workplaceOffsets\":[%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%."
              "3f,%.3f]}"  // WCS offsets Z[0..9]
              "],\"next\":0}\n",
              xh, pos_[0], user_x,  // X axis data
              wcs_offsets_[0][0], wcs_offsets_[1][0], wcs_offsets_[2][0],
              wcs_offsets_[3][0], wcs_offsets_[4][0], wcs_offsets_[5][0],
              wcs_offsets_[6][0], wcs_offsets_[7][0], wcs_offsets_[8][0],
              wcs_offsets_[9][0], yh, pos_[1], user_y,  // Y axis data
              wcs_offsets_[0][1], wcs_offsets_[1][1], wcs_offsets_[2][1],
              wcs_offsets_[3][1], wcs_offsets_[4][1], wcs_offsets_[5][1],
              wcs_offsets_[6][1], wcs_offsets_[7][1], wcs_offsets_[8][1],
              wcs_offsets_[9][1], zh, pos_[2], user_z,  // Z axis data
              wcs_offsets_[0][2], wcs_offsets_[1][2], wcs_offsets_[2][2],
              wcs_offsets_[3][2], wcs_offsets_[4][2], wcs_offsets_[5][2],
              wcs_offsets_[6][2], wcs_offsets_[7][2], wcs_offsets_[8][2],
              wcs_offsets_[9][2]);
          if (written > 0 && written < sizeof(buffer)) {
            response_str = buffer;
          } else {
            LOGE(TAG,
                 "snprintf buffer overflow or error creating M409 move.axes "
                 "response");
            response_str =
                "{\"err\":\"Buffer overflow generating response\"}\n";
          }
          ...
          snprintf(
              buffer, sizeof(buffer),
              "{\"key\":\"move.speedFactor\",\"flags\":\"\",\"result\":%.3f}\n",
              feed_multiplier_);
          response_str = buffer;
          ...
          snprintf(
              buffer, sizeof(buffer),
              "{\"key\":\"move.workplaceNumber\",\"flags\":\"\",\"result\":%d}"
              "\n",
        ...
        response_str =
            "{\"err\":\"Missing or invalid key parameter for M409\"}\n";
      }
    } else {
    ...
    if (!args.empty()) {
      ...
      for (const auto &arg : args) {
        if (!arg.empty() && (arg[0] == 'P' || arg[0] == 'p')) {
          ...
          if (first_quote != std::string::npos &&
              last_quote != std::string::npos && first_quote != last_quote) {
      ...
      if (path == "/macros" || path == "/macros/") {
        response_str =
            "{\"dir\":\"/macros/"
            "\",\"first\":0,\"files\":[\"probe_work.g\",\"free_z.g\",\"all_"
            "zero.g\",\"z_zero.g\",\"zero_workspace.g\",\"work_align_xy10mm."
            "gcode\",\"move_free.g\",\"touch_probe_work.g\"],\"next\":0,"
            "\"err\":0}\n";
      } else if (path == "/gcodes" || path == "/gcodes/") {
        response_str =
            "{\"dir\":\"/gcodes/"
            "\",\"first\":0,\"files\":[\"Updown.gcode\",\"Updown1.gcode\","
            "\"updown 6.1.gcode\",\"MiniNC Z Plate.gcode\",\"updown 6.1 "
            "5mm-adaptive.gcode\",\"updown 6.1 - adaptive 5mm, pocket "
            "0.5mm-0.75mm, slot 0.5mm.gcode\",\"MiniNC Z Plate Contout "
            "Only.gcode\"],\"next\":0,\"err\":0}\n";
      } else {
        // Simulate "directory not found" error
        char buffer[256];
        snprintf(
            buffer, sizeof(buffer),
            "{\"dir\":\"%s\",\"first\":0,\"files\":[],\"next\":0,\"err\":2}\n",
      ...
      response_str =
          "{\"dir\":\"/\",\"first\":0,\"files\":[\"/gcodes\",\"/macros\",\"/"
          "sys\"],\"next\":0,\"err\":0}\n";  // Example root listing
    }
  }
  // Add other command responses as needed...
  else {
    // For most G/M codes that don't explicitly return data, RRF just responds
    // with "ok\n" after execution. We might want to add "ok\n" only if no other
    // JSON response was generated. However, some commands like M409 *do*
    // generate JSON and *don't* add "ok". Let's assume only commands *not*
    // handled above get a simple "ok".
    if (response_str.empty()) {
      // response_str = "ok\n"; // Suppress "ok" for now to match behavior of
      // M409 etc.
    }
  }
...
// Function to add the mock serial instance (implementation depends on how
// serial_instances is managed)
