...
void RRFMachineSimStream::process_gcode(const char *gcode_line) {
  ...
  char *token = strtok_r(
      NULL, " ", &save); // Start parsing arguments from the rest of the line
  while (token != NULL && num_last_args_ < 10) {
    last_args_[num_last_args_++] = token;
    token = strtok_r(NULL, " ", &save);
  }
  ...
  // generate response, so M409 works even without explicit prior commands.
  generate_response(last_command_.c_str(), (const char **)&last_args_,
                    num_last_args_);
...
void RRFMachineSimStream::process_last_command(String last_command_) {
  ...
  if (last_command_ == "G28") {
    ...
    _df(0, ">> G0/G1 %s => %f %f %f (x,y,z) - %f, %f, %f (pos)",
        last_command_.c_str(), x, y, z, pos_[0], pos_[1], pos_[2]);
  } else if (last_command_ == "G90") {
    _df(0, "G90");
    relative = false;
  } else if (last_command_ == "G91") {
...
// Generates "canned" RRF responses, example.
void RRFMachineSimStream::generate_response(const char *command,
    ...
    if (num_args > 0) {
      ...
      if (key == "move.axes" || key == "move.axes[]") {
        ...
        snprintf(
            ...
            "1450,\"drivers\":[\"0.2\"],\"homed\":%s,\"jerk\":300.0,\"letter\":"
            "\"X\",\"machinePosition\":%.3f,\"max\":200.00,\"maxProbed\":false,"
            ...
            "800.00,\"userPosition\":%.3f,\"visible\":true,"
            "\"workplaceOffsets\":[%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%."
            "3f,%.3f]},{\"acceleration\":900.0,\"babystep\":0,\"backlash\":0,"
            "\"current\":1450,\"drivers\":[\"0.1\"],\"homed\":%s,\"jerk\":300."
            "0,\"letter\":\"Y\",\"machinePosition\":%.3f,\"max\":160.00,"
            ...
            "\"visible\":true,\"workplaceOffsets\":[%.3f,%.3f,%.3f,%.3f,%.3f,%."
            "3f,%.3f,%.3f,%.3f,%.3f]},{\"acceleration\":100.0,\"babystep\":0,"
            "\"backlash\":0,\"current\":1450,\"drivers\":[\"0.3\"],\"homed\":%"
            "s,\"jerk\":30.0,\"letter\":\"Z\",\"machinePosition\":%.3f,\"max\":"
            "70.00,\"maxProbed\":false,\"microstepping\":{\"interpolated\":"
            ...
            "00,\"userPosition\":%.3f,\"visible\":true,\"workplaceOffsets\":[%."
            "3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f]}],\"next\":0}\n",
            xh, x, xu, wcs_offsets_[0][0], wcs_offsets_[1][0],
            wcs_offsets_[2][0], wcs_offsets_[3][0], wcs_offsets_[4][0],
            wcs_offsets_[5][0], wcs_offsets_[6][0], wcs_offsets_[7][0],
            wcs_offsets_[8][0], wcs_offsets_[9][0], yh, y, yu,
            ...
            wcs_offsets_[9][1], zh, z, zu, wcs_offsets_[0][2],
            wcs_offsets_[1][2], wcs_offsets_[2][2], wcs_offsets_[3][2],
            wcs_offsets_[4][2], wcs_offsets_[5][2], wcs_offsets_[6][2],
            wcs_offsets_[7][2], wcs_offsets_[8][2], wcs_offsets_[9][2]);
