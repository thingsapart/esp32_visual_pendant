...
void machine_interface_remote_buffer_message(machine_interface_remote_t *self, const uint8_t *data, size_t data_len)
{
    ...
    if (data[0] != MSG_TYPE_BINARY && data_len > sizeof(remote_msg_t))
    { // sizeof(remote_msg_t) is max size for simple types
...
void machine_interface_remote_process_message(machine_interface_remote_t *self, const uint8_t *data, size_t len)
{
    ...
    {
        if (len < sizeof(uint8_t) + sizeof(int) + sizeof(size_t))
        { // type + rpm + tool_name_len
...
#else
bool machine_remote_setup_response_processing_task(machine_interface_remote_t *self, gcode_queue_t *task_event_queue)
#endif
{
    ...
    if (msg_box->num_choices > 0)
    {
        ...
        for (size_t i = 0; i < msg_box->num_choices; ++i)
        {
            ...
            if (choice_offsets_ptr[i] >= header->total_size)
            {
                ...
                { // Free successfully duplicated strings so far
